...
| **macOS, Linux, or Windows** | macOS and Linux via `curl | bash`; Windows via PowerShell `install.ps1` or Git Bash | — |
...
**No Rust, no build — download the prebuilt binary:**
...
> **Windows with Git Bash / MSYS2:** use the `curl | bash` form above.

The installer detects your OS and architecture (macOS, Linux, Windows — arm64 and x86\_64), downloads `ainxt-os` and `ainxt-runtimed` for your platform, places them in `~/.ainxt-os/bin/`, and adds that to your `PATH`. Open a new terminal after install, then:

```sh
ainxt-os        # macOS / Linux / Git Bash
ainxt-os.exe    # Windows cmd / PowerShell
```

That starts AiNxt OS and opens a chat window at **<http://127.0.0.1:8081>**. No database, no message broker, no model provider, no API key.

> **Two binaries, one command.** `ainxt-os` is the **Console** — the browser UI and settings page you interact with. It starts and manages `ainxt-runtimed` for you automatically. `ainxt-runtimed` is the **runtime** — the governed HTTP engine that every AI turn passes through. You only ever run `ainxt-os`; it takes care of `ainxt-runtimed`.
...
| I want to… | My path |
|---|---|
| **Try AiNxt OS now** — no Rust, no build, no toolchain | ↓ Run the one-liner below. Up in two minutes. |
| **Build from source** — audit the code, contribute, or run an unreleased commit | ↓ `git clone` + `./setup.sh` (Rust 1.94+, ~2 GB disk, ~10 min) |
| **Deploy AiNxt OS for a team** — governed, multi-tenant, behind your own gateway | ↓ Install below, then see [Decide how identity is established](#decide-how-identity-is-established--the-daemon-will-not-boot-until-you-do) |
...
> `ainxt-os` is the **Console** — the browser UI and settings page you interact with. It starts and manages `ainxt-runtimed` for you automatically. 

> `ainxt-runtimed` is the **runtime** — the governed HTTP engine that every AI turn passes through. You only ever run `ainxt-os`; it takes care of `ainxt-runtimed`.

**Pin a specific version** (replace `0.2.0` with the version you want):

```sh
curl -fsSL https://raw.githubusercontent.com/npci/ainxt-os/main/install.sh | bash -s 0.2.0
```

**Build from source instead** (Rust 1.94+, ~2 GB disk):

```sh
git clone https://github.com/npci/ainxt-os.git && cd ainxt-os && ./setup.sh --run
```

`./setup.sh --run` builds both binaries, writes a config, validates it, and opens the Console. Run it from inside the cloned `ainxt-os/` directory. On Windows, use Git Bash or WSL2 to run `setup.sh`.
...
### Option 1 — Console Settings: fastest, no file editing
...
ainxt-os                  # if installed via curl
...
### Option 3 — `runtimed.toml`: named providers, permanent
...
| | |
|---|---|
| **Start here** | [Try it — one command](#try-it--one-command) · [Connect a model](#connect-a-model) · [① Console Settings](#option-1--console-settings-fastest-no-file-editing) · [② Env vars](#option-2--environment-variables-one-liner-nothing-to-edit) · [③ runtimed.toml](#option-3--runtimedtoml-named-providers-permanent) · [What AiNxt OS is](#what-ainxt-os-is) · [Your first 10 minutes](#quick-start) · [Where your code stops](#where-your-code-stops-and-the-platform-starts) · [BYOM](#bring-your-own-model-byom) |
...
build (299 crates from crates.io). macOS, Linux, and Windows (use Git Bash or WSL2 to run `setup.sh`). Full detail in [Prerequisites](#prerequisites).
...
| Platforms | **macOS**, **Linux**, and **Windows** (x86\_64 and arm64). The prebuilt binary is the easiest path on all three — `install.ps1` for native Windows, `curl \| bash` for macOS and Linux. Building from source on Windows requires Git Bash or WSL2 to run `setup.sh`; `cargo build` itself works natively with the MSVC toolchain. |
...
flowchart TB
    ...
    DB[("PostgreSQL + Redis<br/><i>only needed by the Platform</i>")]

    CODE -->|"direct, by default"| M
    CLI -->|"direct, by default"| M
    CODE -.->|"optional: governed access"| PLAT
    CLI -.->|"optional: governed access"| PLAT
    PLAT --> DB
    PLAT -.->|"RUNTIME_URL"| RT
...
|---|---|---|---|
| **`ainxt-enterprise`** — AiNxt Platform | The gateway. Python/FastAPI. Serves `/ainxt/v1/api/*` (auth, budgets, skills, admin) and an OpenAI-compatible `/v1/chat/completions`. Ships a React UI. | `8000` (API), `5173` (UI) | Optional. Only needed for a shared, governed, multi-user deployment — CLI and IDE plugin both work standalone with a direct API key. |
| **`ainxt-cli`** — terminal agent | A TUI coding agent, also runs headless for CI. Works standalone with your own API key — no Platform required. | — | Optional. Use it for AI in the terminal; point it at the Platform only if your team runs one. |
| **`ainxt-code`** — IDE plugins | VS Code extension and IntelliJ plugin. Works standalone with a direct API key — no Platform required. | — | Optional. Use it for AI in the editor; point it at the Platform only if your team runs one. |
...
**The one dependency you cannot skip:** at least one model provider somewhere. Nothing in this suite bundles a model. PostgreSQL and Redis are only needed if you additionally choose to run the Platform.
