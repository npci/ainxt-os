...
The `describe` method renders a compact, human-readable string suitable for [`Event::Error`](core_interaction.md).
...
An internal helper that sends an [`Event`](core_interaction.md) to the caller's channel and tracks whether the terminal `Event::Done` has already been emitted. It returns `false` when the receiver has been dropped, allowing the driver to stop early and avoid unnecessary work.
...
|------------|--------|---------|
| `ainxt_protocol::Event` | [`core_interaction`](core_interaction.md) | Canonical event type emitted by the transport. |
...
The SSE transport is a narrow but critical layer in the `ai_engine` → `llm_providers` subsystem. It enables the higher-level prompt engine, runtime, and serving layers to consume LLM responses as a uniform stream of [`Event`](core_interaction.md) objects, regardless of which vendor is actually serving the model.
...
* [`core_interaction`](core_interaction.md) — defines the `Event` type and the protocol events carried by this transport.
* [`prompt_core`](prompt_core.md) — the prompt engineering layer that produces requests consumed by the providers.
* [`core_engine`](core_engine.md) — the runtime engine that routes turns to the appropriate provider.
