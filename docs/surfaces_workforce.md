...
- A creator can turn a plain-language job description into a draft [`RoleSpec`](ainxt_workforce::role::RoleSpec).
...
| [`ModelRoutedExecutor`](surfaces_workforce.md#modelroutedexecutor) | Live, model-backed [`RoleExecutor`](workforce.md#roleexecutor) that drives the Step-7 adversarial run through the daemon's real `ModelRouter`. |
...
- **[workforce](workforce.md)** — the underlying `ainxt-workforce` crate: `RoleStudio`, `Breaker`, `Kernel`, `RoleSpec`, `PublishedRole`, `DigitalTeam`, controls, lifecycle, oversight.
...
- **[tools_cli](tools_cli.md)** — OBO policy (`ThreeLayerPolicy`, `MapAbac`, `OboContext`, `OboDecisionSink`).
- **[ai_engine/evaluation_testing/eval_cases](eval_cases.md)** — `EvalCase` generation for the Step-6 eval battery.
- **[governance_compliance/governance](governance.md)** — `Marketplace`, `PinnedSource`, `AuthoringContext`, CODEOWNERS approvals.
- **[governance_compliance/compliance](compliance.md)** — `StrongRedactor` used for PII detection in `ModelRoutedExecutor`.
- **[core_infrastructure/security_config](security_config.md)** — `DataClass` routing and residency decisions.
- **[core_infrastructure/core_interaction](core_interaction.md)** — `SessionManager`, `Request`, `Event` streaming.
