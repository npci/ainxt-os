...
| `DataClass` | [core_infrastructure](core_infrastructure.md) | Enumerates regulated data classes implicated in the incident. |
| `JsonlEventLog` / `LogRecord` | [core_interaction](core_interaction.md) | Underlying tamper-evident log substrate from which the register's event slice is derived. |
...
- **Sidestream**: It leverages the tamper-evident [eventlog](core_interaction.md) chain (via the register's events) to count evidentiary records, ensuring the draft reflects the current evidence slice.
...
- [core_interaction](core_interaction.md): Event-log and telemetry substrates underlying incident evidence.
- [core_infrastructure](core_infrastructure.md): `DataClass`, `Principal`, and other shared domain types.
