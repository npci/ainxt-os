...
Once admitted, the turn constructs a [`RunAuthorization`](identity.md) rooted at the real authenticated principal and the just-admitted credential. It then authorizes the `chat.send` action. A structurally invalid delegation chain (reserved payment-initiation verb, expiry, cycle) denies the turn fail-closed.
...
- It consumes the shared [`ControlPlane`](identity.md) from the governance/identity subsystem for admission, renewal, and revocation.
...
- It uses [`Principal`](security_config.md) from the types/config layer and [`Request`](core_interaction.md) / [`Event`](core_interaction.md) from the protocol layer.
...
|------------|--------|---------|
| `ainxt_identity::authority` | [identity](identity.md) | `IdentityAuthority`, `AttestationQuote`, `IssueRequest`, `ReferenceValueVerifier` |
| `ainxt_identity::authz` | [identity](identity.md) | `RunAuthorization`, `AuthzDecision` |
| `ainxt_identity::control` | [identity](identity.md) | `ControlPlane`, `DispatchOutcome`, `RunLease` |
| `ainxt_identity::transparency` | [identity](identity.md) | `TransparencyLog`, `IssuanceEntry`, `Sha256Hasher` |
| `ainxt_protocol` | [core_interaction](core_interaction.md) | `Event`, `Request` |
| `ainxt_runtime` | [core_engine](core_engine.md) | `TurnHandler`, `TurnSummary`, `TurnError`, `CancelToken` |
| `ainxt_types` | [security_config](security_config.md) | `Principal` |
...
- [identity](identity.md) — `ControlPlane`, `IdentityAuthority`, `RunAuthorization`, transparency logging.
- [security_config](security_config.md) — `Principal`, types, and configuration primitives.
- [core_interaction](core_interaction.md) — `Request`, `Event`, session protocol.
