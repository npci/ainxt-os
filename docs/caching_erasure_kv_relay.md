...
The relay relies on [`idempotency`](idempotency.md) for exactly-once request semantics:

- [`IdempotencyLedger::begin`](idempotency.md#idempotencyledger) detects duplicate or in-flight requests.
- [`IdempotencyLedger::commit`](idempotency.md#idempotencyledger) bills KV pages exactly once and pins a delivery hash.
- [`IdempotencyLedger::is_committed`](idempotency.md#idempotencyledger) lets the handoff driver short-circuit already-delivered requests before touching the fabric.

See [idempotency.md](idempotency.md) for the full contract, including the divergence guard and drain disposition behavior.
...
- Use a fresh [`IdempotencyLedger`](idempotency.md#idempotencyledger) to verify exactly-once billing and duplicate suppression.
...
- [idempotency.md](idempotency.md) — Exactly-once billing ledger and drain disposition.
