...
- [governance_compliance_identity](governance_compliance_identity.md) — advanced identity, attestation, delegation, and workload credentials build upon `Principal`.
- [pipeline_runtime_runtime_engine](pipeline_runtime_runtime_engine.md) — the engine uses `Principal` for RBAC, outsourcing guard, and admission decisions.
- [ai_engine_knowledge_retrieval](ai_engine_knowledge_retrieval.md) — retrieval ACLs filter by principal clearance, `ad_level`, and `groups`.
...
- For more sophisticated identity concepts such as delegation chains, agent workload credentials, attestation quotes, kill switches, and transparency logs, see [governance_compliance_identity](governance_compliance_identity.md).
