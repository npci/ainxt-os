...
This classification is forwarded to the embedded runtime and client so that downstream guards, routing, and audit policies can enforce the appropriate handling. See [`ainxt_types`](core_infrastructure.md#security_config) for the underlying data-class model.
...
A deterministic, network-free [`Provider`](runtime_engine.md) implementation. It always returns:
...
`OfflineProvider` is registered into a local [`ModelRouter`](runtime_engine.md#modelrouter) so that `run`, `harness dev`, and `harness test` can execute without any external model credentials. Production composition replaces this with real providers loaded from `RuntimeConfig`.
...
- A [`ModelRouter`](runtime_engine.md#modelrouter) containing `OfflineProvider`.
- A default [`Engine`](runtime_engine.md) via `engine_with_defaults`.
- A [`SessionManager`](core_interaction.md) wrapping the engine.
...
The harness is then executed through [`ainxt_admission::HarnessRuntime`](governance_compliance.md#admission) with an [`InMemoryHarnessAudit`](governance_compliance.md#admission) sink. See the [admission module](governance_compliance.md#admission) for details on harness validation, RBAC, and audit.
...
- [`runtime_engine`](runtime_engine.md) for turn execution, routing, and provider abstraction.
- [`core_interaction`](core_interaction.md) for session management.
- [`governance_compliance/admission`](governance_compliance.md#admission) for harness manifests, capability authorization, and offline harness execution.
- [`governance_compliance/governance`](governance_compliance.md#governance) for the publish pre-receive gate and PR descriptor generation.
- [`core_infrastructure`](core_infrastructure.md) for `Principal`, `DataClass`, and protocol primitives.
...
- [`runtime_engine`](runtime_engine.md) — the core execution engine and model router.
- [`core_interaction`](core_interaction.md) — session management and protocol event types.
- [`governance_compliance`](governance_compliance.md) — admission, governance, harness validation, and pre-receive gates.
- [`core_infrastructure`](core_infrastructure.md) — `Principal`, `DataClass`, and shared protocol primitives.
