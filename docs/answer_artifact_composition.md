...
The central abstraction is [`Answer`](crates/ainxt-answer/src/lib.rs::Answer), a serializable structure composed of:
...
Each section body is a list of [`Segment`](crates/ainxt-answer/src/lib.rs::Segment) values in reading order:
...
[`Verbosity`](crates/ainxt-answer/src/lib.rs::Verbosity) is a three-level enum that bounds the composed answer:
...
The level is derived from [`Tier`](core_infrastructure.md) (`Simple`, `Medium`, `Complex`) via [`Verbosity::for_tier`](crates/ainxt-answer/src/lib.rs::Verbosity::for_tier). The bounds are monotone: `Terse ⊆ Normal ⊆ Detailed` on both axes. Truncation is always recorded as a [`CompositionWarning`](crates/ainxt-answer/src/lib.rs::CompositionWarning), never silent.
...
[`ComposedAnswer`](crates/ainxt-answer/src/lib.rs::ComposedAnswer) is the output of [`Answer::compose`](crates/ainxt-answer/src/lib.rs::Answer::compose). It contains the bounded answer, the resolved references, and any warnings. From this value, renderers are pure functions:

- [`to_markdown`](crates/ainxt-answer/src/lib.rs::ComposedAnswer::to_markdown) — GitHub-flavored Markdown with headings, fenced code, pipe tables, and a references section.
- [`to_plain_text`](crates/ainxt-answer/src/lib.rs::ComposedAnswer::to_plain_text) — Markdown-free text for surfaces that cannot render rich syntax.
...
- [`Tier`](core_infrastructure.md) from `ainxt-types` drives verbosity selection.
...
- [application_runtime](application_runtime.md) / [surface_conversation](surface_conversation.md) delivers the rendered text to the user.
...
- **Composition is total**: [`Answer::compose`](crates/ainxt-answer/src/lib.rs::Answer::compose) cannot fail or panic.
...
- [application_runtime](application_runtime.md) — runtime surfaces and conversation delivery.
- [core_infrastructure](core_infrastructure.md) — shared types including `Tier`.
