...
- [`runtime_engine`](runtime_engine.md) constructs an [`ArtifactRuntime`] at startup and exposes `POST /v1/artifact`.
- [`surface_conversation`](surface_conversation.md) may resolve a document-generation turn to a title + body string, then use [`Document::from_text`] to bridge into the artifact IR.
- The capability check uses [`Principal`](security_config.md) from the security config layer.
...
- [`runtime_engine`](runtime_engine.md) — the runtime that hosts and dispatches artifact generation.
- [`surface_conversation`](surface_conversation.md) — conversational surfaces that may invoke document generation.
...
- [`security_config`](security_config.md) — `Principal` and capability model.
