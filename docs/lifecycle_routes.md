...
Both services are intentionally **pure and deterministic**: they accept a logical `now` tick, perform no I/O, use no clocks or RNG, and mutate only their own state. The served daemon (see [runtime_configuration](runtime_configuration.md) / [server_serving_core](server_serving_core.md)) is responsible for durable storage, hydration of cross-tier lineage, and hot-wiring the shared [`RecordStore`] into both services.
...
- [security_config_identity](security_config_identity.md) — `Principal`, `Role`, capability model.
...
`ainxt-runtimed` (see [runtime_configuration](runtime_configuration.md)) is expected to:
...
- [security_config_identity](security_config_identity.md) — `Principal`, `Role`, and capability checks.
- [runtime_configuration](runtime_configuration.md) — daemon configuration and hot-wiring of lifecycle services.
- [server_serving_core](server_serving_core.md) — HTTP route mounting and request handling.
