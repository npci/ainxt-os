...
| Consumer | Usage |
|----------|-------|
| [pipeline_orchestration](pipeline_orchestration.md) | The program verifier calls `SodVerifyGate::authorize_approval` at each commit boundary before treating a produced artifact as approved/committable |
| [runtime_engine](runtime_engine.md) | The runtime engine threads AWCs through turns; SoD gates apply when a Run's output is verified by another Run |
...
[security_config_cryptoagility](security_config_cryptoagility.md) for the broader crypto-agility
