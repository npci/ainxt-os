...
- [`runtime_configuration`](runtime_configuration.md) / [`server_serving`](server_serving.md) (`ainxt-runtimed`, `ainxt-server`) compose the runtime into the served daemon. The `with_oss_defaults` constructor is the recommended served entrypoint because it guarantees the tamper-evident audit floor and requires only the org/dept policy.
