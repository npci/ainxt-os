...
- **Memory**: Long-term memory facts are a required DSAR tier; see [`memory_management`](memory_management.md).
- **Server**: The `RegFiDsarRequest` type in [`server_serving_core`](server_serving_core.md) exposes DSAR operations over the HTTP API.
