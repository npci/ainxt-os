...
- `ainxt-types` — `Principal`, `Role`, `DataClass`, `Tier`.
- `ainxt-runtime` — `ComplianceGate`, `ApprovalGate`, `Redacted`.
- `ainxt-governance` — `Marketplace`, `PinnedSource`, `PrereceiveGate`, `PublishRequest`.
- `serde` — manifest serialization/deserialization.

It is consumed by:

- `ainxt-server` — `/v1/harness/:id` route.
- `ainxt-client` — SDK harness invocation and `WireApprovalGate`.
- `ainxt-cli` — `ainxt harness lint` / `ainxt harness dev`.
