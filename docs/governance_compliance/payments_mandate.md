...
`PaymentAdjacentMandate::issue` refuses unless the signer holds `can_approve` and is senior enough (`ad_level <= 3`). This matches the authority class required by the payment-boundary front-matter subsystem (see [`payments_front_matter`](payments_front_matter.md)).
...
It depends conceptually on [`identity`](identity.md) for OBO outcomes, but not structurally (the `OboOutcome` model keeps the dependency acyclic). It is consumed by higher-level dispatch surfaces such as [`runtime_engine`](../pipeline_runtime/runtime_engine.md) and [`server_serving`](../pipeline_runtime/server_serving.md) when authorizing adjacent writes near the payment boundary.
...
- [`runtime_engine`](../pipeline_runtime/runtime_engine.md) — runtime dispatch surfaces that consume mandates
- [`server_serving`](../pipeline_runtime/server_serving.md) — serving layer where adjacent writes are dispatched
