...
`IssueRequest` carries the candidate facets for a new AWC. `ActorRecord` is the composite actor identity written to the event log — never a service account, never a bare role name. `AgentWorkloadCredential::actor_of_record` and `actor_label` produce the court-grade attribution the [`eventlog`](../core_infrastructure/core_interaction.md#event-logging) subsystem records.
...
- **Security config**: `DataClass` from [`security_config_identity`](../core_infrastructure/security_config_identity.md) drives kill-switch scope and TEE requirements.
- **Core interaction**: [`core_interaction`](../core_infrastructure/core_interaction.md) event logging consumes `ActorRecord` for attribution.
- **Serving infrastructure**: [`server_serving`](../pipeline_runtime/server_serving.md) receives `PreemptDirective`s and manages checkpointing.
- **Runtime engine**: [`runtime_engine`](../pipeline_runtime/runtime_engine.md) calls `issue`/`renew` and routes Runs.
...
- [`security_config_identity`](../core_infrastructure/security_config_identity.md) — `DataClass` and `Principal`.
- [`core_interaction`](../core_infrastructure/core_interaction.md) — event logging and session infrastructure.
- [`runtime_engine`](../pipeline_runtime/runtime_engine.md) — engine that drives Runs and calls the AIA.
- [`server_serving`](../pipeline_runtime/server_serving.md) — serving scheduler that consumes preemption directives.
