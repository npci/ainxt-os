...
- [`security_config_identity`](../core_infrastructure/security_config_identity.md) supplies `DataClass` for regulated-data checks.
...
- [`runtime_engine`](../pipeline_runtime/runtime_engine.md) surfaces call `competency_route` and `evaluate_decoy` at approval-dispatch time.
- [`server_serving_core`](../pipeline_runtime/server_serving_core.md) exposes workforce state endpoints that reflect the nightly sweep findings.
...
- [`runtime_engine`](../pipeline_runtime/runtime_engine.md) — engine that consumes competency and decoy outcomes
- [`server_serving_core`](../pipeline_runtime/server_serving_core.md) — server state and workforce HTTP endpoints
- [`security_config_identity`](../core_infrastructure/security_config_identity.md) — identity and data-class primitives
