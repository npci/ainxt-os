...
Governance sits upstream of the runtime: it decides what definitions are allowed to reach `Approved` / `Production`, while the runtime engine ([runtime_engine](../pipeline_runtime/runtime_engine.md), [server_serving](../pipeline_runtime/server_serving.md)) enforces redaction, admission, and serving gates at execution time.
...
- **Testing**: Adversarial and scenario testing in [`scenario_service`](../scenario_service/scenario_service.md) and [`injection_service`](../injection_service/injection_service.md) can reference governed definitions by their pinned hashes.
