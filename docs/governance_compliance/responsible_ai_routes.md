...
| HTTP route mounting, auth, and server state | ❌ | [`server_serving_core`](../pipeline_runtime/server_serving_core.md), [`runtime_configuration`](../pipeline_runtime/runtime_configuration.md) |
| Principal / capability model | ❌ | [`security_config_identity`](../core_infrastructure/security_config_identity.md) |
...
Capability string admitting the model-risk / quality-breaker read surface. It is checked by the served daemon's auth layer before any inventory lookup. An admin principal implicitly holds it via [`Principal::has_cap`](../core_infrastructure/security_config_identity.md).
...
* [`security_config_identity`](../core_infrastructure/security_config_identity.md) — supplies `Principal` and the capability model that interprets `CAP_MODEL_RISK`.
* [`server_serving_core`](../pipeline_runtime/server_serving_core.md) — mounts the model-risk preview endpoints and the promotion admission route.
* [`runtime_configuration`](../pipeline_runtime/runtime_configuration.md) — holds the live `AssembledFull` state, including the shared `quality_breaker` and `dpia_gate` instances.
...
* The capability is enforced at the transport/auth layer, not inside this crate. This module only defines the constant so that [`server_serving_core`](../pipeline_runtime/server_serving_core.md) and [`security_config_identity`](../core_infrastructure/security_config_identity.md) agree on the string.
...
* [`security_config_identity`](../core_infrastructure/security_config_identity.md) — `Principal` and capability enforcement.
* [`server_serving_core`](../pipeline_runtime/server_serving_core.md) — HTTP route mounting and `FullApp`/`FullAppExt` surface.
* [`runtime_configuration`](../pipeline_runtime/runtime_configuration.md) — `AssembledFull` and live governance organ wiring.
