...
| Quality scores | [`quality_verification`](../ai_engine/quality_verification.md) | Source of `MonitoringScoreboard::latest_score` |
| Evaluation signals | [`evaluation_testing`](../ai_engine/evaluation_testing.md) | Offline eval and canary signals that feed monitoring |
...
- **Runtime Serving**: The `QualityCircuitBreaker` provides typed trip facts to [`runtime_engine`](../pipeline_runtime/runtime_engine.md) and [`server_serving`](../pipeline_runtime/server_serving.md), which contain the route and escalate.
- **Quality Verification**: `MonitoringScoreboard::latest_score` is produced by the AI engine's quality verification subsystem. See [`quality_verification`](../ai_engine/quality_verification.md).
- **Evaluation & Testing**: Offline eval cases, canaries, and replay provide the signals that populate monitoring scoreboards. See [`evaluation_testing`](../ai_engine/evaluation_testing.md).
...
- **Identity & Security**: `DataClass` and provenance rules align with [`security_config_identity`](../core_infrastructure/security_config_identity.md) and ADR-012 data-localization requirements.
