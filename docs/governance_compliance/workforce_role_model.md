...
- [`security_config_identity`](../core_infrastructure/security_config_identity.md) / `ainxt-types` for `DataClass` and `Principal`.
- [`governance`](governance.md) / `ainxt-governance` for `GovernanceState` and lifecycle transitions.
- [`skill_execution`](../core_infrastructure/skill_execution.md) for skill references and execution semantics.
- [`connectors`](../core_infrastructure/connectors.md) for connector references and capability grants.
- [`knowledge_retrieval`](../ai_engine/knowledge_retrieval.md) for knowledge-scope namespaces and retrieval quality.
...
Downstream, [`workforce_runtime_teams`](workforce_runtime_teams.md) and the [`runtime_engine`](../pipeline_runtime/runtime_engine.md) surfaces invoke only published (or validated shadow-case) roles, so the guarantees established at authoring time propagate into execution.
