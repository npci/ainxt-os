...
This module is intentionally pure — it contains no async, clock, or threading code. The live async scheduler that binds it to the served runtime lives in [runtime_engine](../pipeline_runtime/runtime_engine.md) (specifically `ainxt-runtimed::workforce_surface`), while the multi-agent task loop that executes team plans lives in [teams](teams.md). The kernel and team structures here provide the deterministic state machine and validation invariants that those runtime surfaces rely on.
...
- **Downstream**, it is consumed by [runtime_engine](../pipeline_runtime/runtime_engine.md) (`ainxt-runtimed::workforce_surface`), which wraps the pure `Kernel` in async scheduling, persists published roles and assembled teams, and binds role execution to the [runtime_engine](../pipeline_runtime/runtime_engine.md) `Engine`.
...
For the live scheduling, turn execution, and HTTP surface that expose these primitives, see [runtime_engine](../pipeline_runtime/runtime_engine.md). For the task-graph planning and self-healing execution loop, see [teams](teams.md).
