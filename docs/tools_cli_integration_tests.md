...
These tests are designed to be run against a running `ainxt-runtimed` instance and exercise the full stack from the HTTP surface through [session management](core_infrastructure.md), [conversation orchestration](core_infrastructure.md), [prompt assembly](ai_engine.md), and [guardrails](ai_engine.md). They complement the in-process Rust tests in [tools_cli](tools_cli.md) and the evaluation harnesses in [ai_engine evaluation_testing](ai_engine.md) by validating real-world conversational behavior.
...
A plain declarative statement (`"I like pizza."`) must be acknowledged, not met with a clarify fallback such as `"I didn't quite catch that"`. This guards the [ClarifyPolicy](ai_engine.md) fallback behavior.

### Scenario 4 — Identity Question Not Refused

Asking `"What is AiNxt?"` must produce a helpful answer rather than a refusal referencing "internal instructions." This guards the L4 guard-body behavior in [guardrails](ai_engine.md).

### Scenario 5 — Cross-Session Isolation

Establishes a fact in session A, then asks the same recall question in session B. Session B must not know the answer, validating that session-scoped cache keys and conversation state are properly isolated. This depends on correct behavior from the [session](core_infrastructure.md) and [cache](core_infrastructure.md) subsystems.
...
For a detailed breakdown of the runtime surface, see [pipeline_runtime runtime_engine](pipeline_runtime.md) and [pipeline_runtime server_serving](pipeline_runtime.md). For conversation and session internals, see [core_infrastructure core_interaction](core_infrastructure.md) and [core_infrastructure application_runtime surface_conversation](core_infrastructure.md).
...
|------------|--------|-------------------|
| `ainxt-runtimed` | [pipeline_runtime](pipeline_runtime.md) | The daemon exposing `/v1/chat`. |
| Session management | [core_infrastructure core_interaction](core_infrastructure.md) | Creates and isolates session state. |
| Conversation orchestration | [core_infrastructure application_runtime surface_conversation](core_infrastructure.md) | Maintains turn history and injects it into prompts. |
| Guardrails | [ai_engine safety_guardrails](ai_engine.md) | Determines whether inputs/outputs are allowed or clarified. |
| Prompt engine | [ai_engine prompt_engineering](ai_engine.md) | Assembles the final prompt including history. |
...
For more comprehensive evaluation and benchmarking, see [ai_engine evaluation_testing](ai_engine.md). For runtime configuration and serving internals, see [pipeline_runtime](pipeline_runtime.md).
