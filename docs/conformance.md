...
- Run 1,000+ generated scenarios through the **real** [`ainxt_runtime::Engine`](runtime_engine.md).
...
|------------|------|----------------|
| [conformance_runtime](conformance_runtime.md) | `src/lib.rs` | Assembles the real runtime into a [`Target`](scenario_service_core.md) and runs the scenario matrix. |
...
[`ConformanceTarget`](conformance_runtime.md) builds a production-like engine and implements the [`Target`](scenario_service_core.md) trait from [`ainxt_scenario`](scenario_service.md). For each [`Scenario`](scenario_service_core.md) it runs a real turn through:

1. **Output redaction** via [`StrongRedactor`](compliance.md).
2. **Capability authorization** via [`RbacAuthorizer`](runtime_engine.md).
3. **Provider failover** â€” a [`FlakyPrimary`](conformance_runtime.md) provider always fails, forcing the router to the [`ConformanceProvider`](conformance_runtime.md).
4. **Tool execution** â€” real [`ToolRuntime`](tools_cli.md) with idempotent [`SettleTool`](conformance_runtime.md) and structured [`PayTool`](conformance_runtime.md).
5. **Injection taint-gating** â€” tainted turns refuse side-effecting tools.
...
- `run_matrix()` â€” full generated matrix.
- `run_pairwise_matrix()` â€” pairwise-generated corpus.
- `ConformanceTarget::run_cancelled()` â€” pre-cancelled turn.
- `ConformanceTarget::run_many_concurrent()` â€” concurrent session isolation.
...
- `dogfood_merge_check()` â€” null change.
- `dogfood_merge_check_with_regression(regression)` â€” negative control.
...
- **[ainxt_runtime](runtime_engine.md)** â€” the engine, router, RBAC authorizer, audit, and provider trait.
- **[ainxt_scenario](scenario_service.md)** â€” scenario definitions, matrix generation, oracles, and the `Target` trait.
- **[ainxt_tools](tools_cli.md)** â€” tool runtime, ledger, and tool trait.
- **[ainxt_protocol](core_interaction.md)** â€” request/event protocol.
- **[ainxt_injection](safety_guardrails_injection.md)** â€” injection detection and taint-gate configuration.
- **[ainxt_compliance](compliance.md)** â€” `StrongRedactor` output compliance gate.
- **[ainxt_types](security_config_identity.md)** â€” `Principal` and `DataClass`.
- **[ainxt_eval](evaluation_testing.md)** â€” release gate, judge calibration, sealed corpus, and regression vault.
