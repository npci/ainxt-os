...
`context_sources` lives under [`ai_engine`](ai_engine.md) â†’ [`knowledge_retrieval`](knowledge_retrieval.md). It feeds [`context_retrieval_routing`](context_retrieval_routing.md), which routes and compiles the resulting chunks into served context windows.
...
- [`security_config`](security_config.md) / `ainxt-types`: provides `DataClass` sensitivity labels that drive both artifact routing and fabric chunk classification.
