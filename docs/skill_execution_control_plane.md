...
The existing `SkillExecutor` (native, WASM, or dispatching) is never disturbed by a reload. Only the registry is swapped, which is held behind an `ArcSwap` in `SkillRuntime`. This design is shared with the prompt control plane; see [prompt_core.md](prompt_core.md) for the analogous pattern.
...
The skill control plane sits at the boundary between version-controlled configuration and the live application runtime. It is typically invoked by `ainxt-runtimed` at startup (via the `[server] skill_dir` configuration key) and by `ainxt-server` admin reload endpoints. The loaded skills are consumed by chat surfaces and turn pipelines; see [surface_conversation.md](surface_conversation.md) and [runtime_engine.md](runtime_engine.md).
...
| `ControlPlane` (prompt) | [prompt_core.md](prompt_core.md) | Pattern this module mirrors. |
...
For broader governance concerns such as admission gates, harnesses, and approval workflows, see [governance_compliance.md](governance_compliance.md).
...
- [prompt_core.md](prompt_core.md) — The prompt control plane pattern this module mirrors.
...
- [runtime_engine.md](runtime_engine.md) — The runtime engine that hosts the skill runtime.
