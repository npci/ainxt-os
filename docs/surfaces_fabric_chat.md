# `surfaces_fabric_chat` â€” Fabric-Grounded Chat Surface
...
`FabricGroundedChatSurface` is the wire: a [`TurnHandler`](runtime_engine.md) wrapper that, on every turn, routes the query through a populated [`MultiGraphFabric`](knowledge_retrieval.md) and prepends the routed, layer-labelled evidence onto the turn as an explicit context block. It is:

- **Additive and config-selectable** â€” it does not replace the default surface assembly.
- **Transparent over an empty fabric** â€” an unpopulated fabric is a byte-identical no-op.
- **Never a turn-admission gate** â€” if the fabric routes to nothing for a turn, the inner handler runs unchanged.
- **Raw-turn preserving** â€” the original user input is kept in `Request::user_turn` so intent classification and referent resolution still operate on the user's own words.

This module lives under the [`pipeline_runtime` â†’ `runtime_engine` â†’ `surfaces`](runtime_engine.md) branch of the system, alongside [`surfaces_chat_identity`](surfaces_chat_identity.md), [`surfaces_workforce`](surfaces_workforce.md), and [`surfaces_prompt_optimizer`](surfaces_prompt_optimizer.md).
...
| `MultiGraphFabric` | The populated fabric-of-graphs maintained by the context/routing layer. See [`knowledge_retrieval`](knowledge_retrieval.md). |
...
- It wraps the normal profiled chat surface (`ProfiledSurface` â†’ `ChatSurface` â†’ `ConversationManager`).
...
1. **Receive turn** â€” `FabricGroundedChatSurface::handle_turn` is invoked with the original `Request`.
2. **Empty-fabric short-circuit** â€” If `fabric.is_empty()`, the request is passed through unchanged. This preserves byte-identical behavior when no indexer has populated a fabric.
3. **Compile fabric** â€” `compile_served_fabric` routes the query through the fabric, applying access control and model eligibility.
4. **Empty-window short-circuit** â€” If the routed window has no chunks, no community summaries, and no artifacts, the request is passed through unchanged. The fabric is a read-filter, not an admission gate.
5. **Render context** â€” Evidence is formatted into a labelled block. Multimodal artifacts are gated through `served_multimodal_turn` against the default artifact fleet; ineligible artifacts are silently dropped.
6. **Preserve raw turn** â€” `Request::user_turn` is set to the original input if not already set.
7. **Delegate** â€” The grounded request is handed to the inner handler, which continues with normal chat processing.
...
- [`surfaces_chat_identity`](surfaces_chat_identity.md) â€” provides identity-governed chat surfaces; `FabricGroundedChatSurface` can wrap the same inner handlers.
- [`runtime_engine`](runtime_engine.md) â€” supplies the `TurnHandler` trait, `Engine`, model routing, and `CancelToken` semantics.
- [`server_serving`](server_serving.md) â€” `ainxt-server` assembles the final surface graph and dispatches HTTP requests to it.

### Downstream Dependencies

- [`knowledge_retrieval`](knowledge_retrieval.md) â€” owns `MultiGraphFabric`, `RoutedWindow`, `FabricGraph`, and the routing/optimization logic.
- [`application_runtime` / `surface_conversation`](application_runtime.md) â€” provides `ChatSurface`, `ConversationManager`, `TurnPlan`, and `SurfaceScopedAuthorizer`.
- [`core_interaction`](core_interaction.md) â€” defines the `Request`, `Event`, `Principal`, and session protocols used across the turn boundary.
- [`security_config`](security_config.md) â€” provides principal identity, access tokens, and configuration loading used when building the access context and eligible model set.

### Composition Roots

The module exposes two assembly helpers in `ainxt-runtimed::lib`:

- `assemble_chat_fabric_grounded` â€” builds a fabric-grounded chat surface from the KB plus optional code graph/content. No artifact tier is attached (air-gapped default).
- `assemble_chat_fabric_grounded_with_artifacts` â€” same, but attaches an `ArtifactStore` to enable the multimodal artifact tier.
...
- `r19_fabric_grounded_chat_served.rs` in `crates/ainxt-runtimed/tests/`
  - Proves **transparency over an empty fabric** â€” byte-identical no-op.
  - Proves **live-wiring** â€” a populated fabric actually routes evidence onto the served turn.
...
- [`runtime_engine`](runtime_engine.md) â€” core turn execution, `TurnHandler`, `Engine`, model routing.
- [`surfaces_chat_identity`](surfaces_chat_identity.md) â€” identity-governed chat surface sibling.
- [`surfaces_workforce`](surfaces_workforce.md) â€” workforce/role-invocation surface sibling.
- [`surfaces_prompt_optimizer`](surfaces_prompt_optimizer.md) â€” prompt-optimization surface sibling.
- [`knowledge_retrieval`](knowledge_retrieval.md) â€” context fabric, routing, retrieval, and artifact stores.
- [`application_runtime`](application_runtime.md) â€” chat surface, conversation manager, turn planning, surface profiles.
- [`core_interaction`](core_interaction.md) â€” protocol types: `Request`, `Event`, `Principal`, sessions.
- [`security_config`](security_config.md) â€” principal, access context, and runtime configuration.
- [`server_serving`](server_serving.md) â€” HTTP server assembly and request dispatch.
