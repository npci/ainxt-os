...
- a `BiasReport`
- an `approver` [`Principal`](security_config_identity.md)
- an `approval_note`
...
`can_approve(principal)` checks for the `APPROVE_CAP` capability or admin status. See [`security_config_identity`](security_config_identity.md) for `Principal` details.
...
- `permitted_data_class` — maximum [`DataClass`](security_config_identity.md) the model may carry
- `intended_use`
- `risk_class`
...
|------------|--------|---------|
| `Principal` | [`security_config_identity`](security_config_identity.md) | Approver identity and capability checks |
...
- **Runtime Serving**: The `QualityCircuitBreaker` provides typed trip facts to the serving/runtime layer, which can contain the route and escalate. See [`runtime_engine`](runtime_engine.md) and [`server_serving`](server_serving.md).
- **Evaluation & Quality**: Bias reports and monitoring scoreboards consume evaluation signals produced by the AI engine's quality verification subsystem. See [`quality_verification`](quality_verification.md).
