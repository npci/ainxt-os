...
|------|----------------|---------------|
| Coarse answer cache | Final response embeddings / answers | [`ainxt-cache`](../core_infrastructure/core_interaction.md) `PartitionedCache` |
...
- **Data class source:** `DataClass` comes from [`ainxt-types`](../core_infrastructure/security_config_identity.md). The sensitivity ordering is defined there.
- **Answer/prompt-prefix cache:** [`ainxt-cache`](../core_infrastructure/core_interaction.md) `PartitionedCache` uses the rendered `PartitionKey` as its partition token.
