...
- `stuck` — optional `(window, threshold)` for the [`StuckDetector`](../ai_engine/quality_verification_judge.md).
...
- `stuck` — optional [`StuckDiagnosis`](../ai_engine/quality_verification_judge.md) when the thrash detector fired.
...
| `ReviewSeams` | Stage 9 LLM Review (finder) + independent Judge panel (adjudicator). | [`quality_verification_judge`](../ai_engine/quality_verification_judge.md) |
...
- [`quality_verification_judge`](../ai_engine/quality_verification_judge.md) supplies `Reviewer`, `JudgePanel`, `JudgeCriteria`, `ReviewFinding`, `PanelVerdict`, `StuckDetector`, and `StuckDiagnosis`.
