...
| `config` | A `SelfHealConfig` that encodes language, risk tier, rung, gate policy, and scoring parameters. `max_rounds` and `stuck` are ignored because a review is a single pass. |

The struct uses `#[serde(deny_unknown_fields)]` to reject smuggled keys at the wire boundary, mirroring the policy applied to `EditRequest`.
...
The public editing entry point. It delegates to `run_edit_turn_full`, forwarding the supplied `EditTurn`, coder, stage tools, SAST scanner, optional performance config, optional review seams, workspace sink, and journal. The function is the only supported path for a surface to *change* code.
...
4. Computes a `ConfidenceScore` that exactly matches the score the gate would consume.
...
|------------|------------|---------------------|
| `crate::edit_turn::{run_edit_turn_full, EditTurn, TurnOutcome}` | Edit Turn Execution | Full editing turn orchestration and outcome types. |
...
| `crate::selfheal::{Coder, ReviewSeams, SelfHealConfig}` | Self Healing | Coder abstraction, review seams, and review configuration. |
| `crate::confidence::{compute, ConfidenceInputs, ConfidenceScore}` | Classification and Risk | Confidence computation consumed by the gate. |
| `crate::risk::RiskTier` | Classification and Risk | Risk tier used in default review config. |
| `crate::perf::PerfConfig` | Performance | Optional performance analysis configuration. |
| `crate::journal::Journal` | Journaling | Hash-chained audit journal. |

### External Crates

| Dependency | Module Doc | Role in this Module |
|------------|------------|---------------------|
| `ainxt_judge::{actionable_review, CoderSubmission, PanelVerdict, ReviewFinding}` | [Quality Verification Judge](../ai_engine/quality_verification_judge.md) | LLM review finder and independent judge panel. |
...
- The CLI and client SDKs in [`tools_cli`](../tools_cli/tools_cli.md).
- The workforce and role surfaces in [`workforce_runtime_teams`](../governance_compliance/workforce_runtime_teams.md) and [`teams`](../governance_compliance/teams.md).

It relies on the semantic-edit engine in [`edit_semantic`](edit_semantic.md) for workspace writes and rung levels, and on the quality verification subsystem in [`quality_verification_judge`](../ai_engine/quality_verification_judge.md) for the LLM review finder and judge panel.
...
- Edit Turn Execution — full editing turn implementation.
...
- Self Healing — coder, self-heal config, and review seams.
- [SAST](pipeline_stages_and_tools_sast.md) — static analysis integration.
- Classification and Risk — confidence scoring.
- Journaling — hash-changed audit journal.
- [Quality Verification Judge](../ai_engine/quality_verification_judge.md) — `ainxt-judge` finder and panel.
