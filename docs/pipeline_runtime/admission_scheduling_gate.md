...
`ServingGate` implements `ainxt_identity::authority::PreemptionSink`. When the identity plane's `KillSwitch` signals preemption for a `run_id`, the gate's `force_preempt_run` finds any running sequence carrying that `run_id` on its live scheduler and preempts it. This gives the kill switch reach into already-admitted runs, not just future issuances. See [`identity`](../governance_compliance/identity.md) for the identity-plane side of this contract.
...
- [`identity`](../governance_compliance/identity.md) — identity plane and kill-switch preemption.
