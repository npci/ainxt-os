...
|---|---|
| **supervisor** | The Program Supervisor drives the LONG_HORIZON-era `Program`/`NodeDecl` graph (event-sourced, durable). This module wraps the LOOP-era `Plan`/`Step` graph — a structurally different type. See [§6 Current Status](#6-current-status--gap-audit) below. |
| **verification** | The three-way gate (deterministic + adversarial + Judge) that proves a module "done." This module's `RevisableExecutor` seam is where a real base-loop Run (backed by the verification gate) would plug in. |
...
sequenceDiagram
    ...
    alt churn leq threshold
        ...
        LoopActor->>LoopActor: froze = true#59; break
    end
```
...
| supervision_and_verification_supervisor | The Program Supervisor loop that drives the LONG_HORIZON-era `Program` graph. |
| supervision_and_verification_verification | The three-way verification gate (deterministic + adversarial + Judge). |
