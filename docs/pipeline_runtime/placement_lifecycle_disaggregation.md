...
- `admit_prefill` admits a prefill-phase request against only the prefill gate.
- `admit_decode` admits a decode-phase request against only the decode gate. Because it touches a separate [`ServingGate`](admission_scheduling.md#servinggate) instance, it is structurally independent of prefill-pool saturation.
- `complete_prefill` and `complete_decode` free slots in their respective pools without cross-pool side effects.

### KV Handoff

`handoff` moves finished prefill KV blocks to a decode node via [`prefill_to_decode_handoff`](caching_erasure.md#prefill_to_decode_handoff). The handoff is:
...
|-----------|------|---------------|
| `DisaggregatedPools` | Composes two independent gates and the KV relay | `ServingGate`, `KvRelay`, `IdempotencyLedger` |
| `FakeExecutor` | Test-only stub implementing `InferExecutor` | `InferRequest`, `StreamHandle` |
