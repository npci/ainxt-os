...
principal and a [`Request`](../core_infrastructure/core_infrastructure.md), routes it through mandatory
...
`Engine` is the central struct. It is
built with the three mandatory gates and a [`ModelRouter`](#model-router), then
extended through a fluent `with_*` API:

- `with_tools` / `with_shared_tools` — attach a [`ToolRuntime`](../tools_cli/tools_cli.md) for the agent loop.
...
| [memory_management](../ai_engine/memory_management.md) | Supplies `InMemoryStore`, durable memory backends, and the Context-Fabric read path used by `SharedMemoryStore`. |
| [safety_guardrails](../ai_engine/safety_guardrails.md) | `GuardrailsConfig`, `RailChain`, and `InjectionConfig` feed the input/output rails and prompt-injection defense. |
| [prompt_engineering](../ai_engine/prompt_engineering.md) | Complexity classifiers, prompt cache, and steerability eligibility are configured here and handed to the engine. |
| [tools_cli](../tools_cli/tools_cli.md) | `ToolRuntime`, OBO policy / sink, exactly-once ledger, and tool schema validation live in this area. |
| [core_infrastructure](../core_infrastructure/core_infrastructure.md) | Foundational types: `Principal`, `DataClass`, `Tier`, `Request`, `Event`, `EventEnvelope`, telemetry, and protocol errors. |
| [ai_engine](../ai_engine/ai_engine.md) | Quality verification, judge panels, and synthesis feed the live quality scoreboard that the router consumes via `live_quality_metrics`. |
| [governance_compliance](../governance_compliance/governance_compliance.md) | Outsourcing register, model-risk records, responsible-AI gates, and identity/attestation policies inform router eligibility and node attestation. |
...
- [memory_management](../ai_engine/memory_management.md)
- [safety_guardrails](../ai_engine/safety_guardrails.md)
- [prompt_engineering](../ai_engine/prompt_engineering.md)
- [tools_cli](../tools_cli/tools_cli.md)
- [core_infrastructure](../core_infrastructure/core_infrastructure.md)
- [ai_engine](../ai_engine/ai_engine.md)
- [governance_compliance](../governance_compliance/governance_compliance.md)
