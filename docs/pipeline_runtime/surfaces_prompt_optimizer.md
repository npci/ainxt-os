...
2. **Prompt Engineering** — It consumes the optimization primitives defined in [prompt_optimization](../ai_engine/prompt_optimization.md) and the prompt registry/layering primitives in [prompt_core](../ai_engine/prompt_core.md). It does not reimplement optimization; it only provides the live driver and provider adapters.

3. **Evaluation & Providers** — It uses `EvalCase` and `QualityJudge` from [evaluation_testing](../ai_engine/evaluation_testing.md) and the `Provider` trait from [llm_providers](../ai_engine/llm_providers.md) / [core_engine](core_engine.md).
...
|-------|------------|-------|
| `ainxt_eval` | [evaluation_testing](../ai_engine/evaluation_testing.md) | `EvalCase`, `QualityJudge`, scoring |
| `ainxt_prompt` | [prompt_core](../ai_engine/prompt_core.md) | `Registry`, `Layer`, `ModelFamily`, `PromptVariant`, `ConstrainedDecoder` |
| `ainxt_promptopt` | [prompt_optimization](../ai_engine/prompt_optimization.md) | Optimization, bridge, budget, holdout, A/B promotion, proposal |
| `ainxt_protocol` | [core_interaction](../core_infrastructure/core_interaction.md) | `Event` streaming contract |
| `ainxt_runtime` | [core_engine](core_engine.md) | `Provider` trait |
| `ainxt_types` | [core_infrastructure](../core_infrastructure/core_infrastructure.md) | `Tier` |
...
- [prompt_optimization](../ai_engine/prompt_optimization.md) — the underlying optimizer engine.
- [prompt_core](../ai_engine/prompt_core.md) — prompt registry, layers, and lifecycle.
...
- [evaluation_testing](../ai_engine/evaluation_testing.md) — eval cases, judges, and quality scoring.
- [llm_providers](../ai_engine/llm_providers.md) — provider adapters and the `Provider` trait.
