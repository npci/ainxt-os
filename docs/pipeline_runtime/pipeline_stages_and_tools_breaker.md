...
The Breaker is deliberately **not** a universal gate. It runs only when the preceding classification and risk stage assigns a `RiskTier::HighRisk` verdict. For lower-risk edits, the Breaker is *not consulted*, and the absence of a result is reported honestly as "not run" rather than a false clean.
...
- pipeline_stages_and_tools_classification_and_risk — produces the `RiskTier` that gates Breaker execution.
...
- pipeline_stages_and_tools_classification_and_risk
...
- [scenario_service_breaker](../scenario_service/scenario_service_breaker.md)
