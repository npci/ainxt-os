...
- **Durable, event-sourced state** â€” every state change is an append-only, hash-chained event; the live projection is folded from the log.
- **Resumability** â€” a program can stop on Friday and resume on Monday by replaying its durable log, never re-executing already-committed work.
- **Three-way verification** â€” no node is considered done until independent deterministic, adversarial, and cross-model semantic verdicts all pass.
- **Program-scale verification** â€” the whole program is declared `Completed` only after per-edge integration checks, a regression sweep, and an independent program-level judge all pass.
- **Honest partial completion** â€” when a node cannot be proven, the program seals a `CappedPartial` outcome and produces a deployable report of what *did* commit.
- **Single-module rollback** â€” a just-committed node that breaks an already-good neighbor can be rolled back, cascading only its own committed dependents.
- **Poison-node quarantine** â€” a persistently failing node is isolated so independent branches can keep progressing.
- **Child-program composition** â€” a node can spawn a nested Program, blocking the parent until the child reaches a terminal outcome.
- **Edit-ladder floor enforcement** â€” critical-path nodes can forbid low-rung (e.g. raw text-patch) edits even when the three-way gate is green.
...
Every node in a program carries a contract derived from the ADR-027 Â§3 specification:

- `id` and `node_class` (`MigrationRun`, `Shim`, `Integration`, `ChildProgram`, â€¦)
- `deps` â€” dependency graph edges
- `blast_radius` â€” dependents from the call/import graph
- `working_set_estimate` â€” token budget for admissibility checks
- `verification_plan` â€” seams/tests that gate the node
- `checkpoint_class` â€” whether a human gate is required
- `edit_ladder_floor` â€” minimum safe Semantic-Editing rung (`Lsp`, `Ast`, `StructuredPatch`, `TextPatch`)
...
1. **Deterministic verdict** â€” compiles, tests pass, no SAST hard-blocks.
2. **Adversarial verdict** â€” no Breaker counterexamples.
3. **Judge verdict** â€” cross-model semantic review above threshold and completed.
...
- [`Program::resume`](program_execution_driver.md) â€” full replay from the log.
- [`Program::resume_from_checkpoint`](program_execution_driver.md) â€” replay only the tail onto a [`ProgramCheckpoint`](program_execution_driver.md) snapshot.
...
- **[`plan_definition`](plan_definition.md)** â€” supplies the `NodeDecl` graph, `ModuleRef` identities, and `Plan` lifecycle escalation used by the driver.
- **[`supervision_and_verification`](supervision_and_verification.md)** â€” supplies the `three_way_gate`, `program_completed`, and `ProgramVerifier` abstractions that the driver calls.
- **[`program_governance_and_execution`](program_governance_and_execution.md)** â€” the runtime layer that hot-wires real `ModuleExecutor`, `ModuleJudge`, and compensator implementations onto the pure driver seams.
- **[`edit_semantic`](edit_semantic.md)** â€” provides the Semantic-Editing ladder (`Lsp`, `Ast`, â€¦) whose rung is enforced at the commit gate.

## See Also

- [`program_execution_state.md`](program_execution_state.md) â€” detailed documentation of the event-sourced aggregate.
- [`program_execution_driver.md`](program_execution_driver.md) â€” detailed documentation of the live-drivable Program API and verified drive loop.
