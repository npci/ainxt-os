...
### `IndexState` â€” the event-driven tracker
...
### `IndexEntry` â€” per-node bookkeeping

`IndexEntry` is a private struct holding:

- `fingerprint: u64` â€” the FNV-1a hash of the text last indexed.
- `indexed_tick: i64` â€” the logical tick at which the node was last indexed.

The tick is passed in by callers; no wall clock is used, satisfying the system's `DETERMINISTIC` mandate.

### `ReindexTrigger` â€” minimal rebuild instructions

Each trigger names one node and indicates whether an embedding is required:

- `Added { id }` â†’ index + embed the new node.
- `Changed { id }` â†’ re-index + re-embed because content changed.
- `Removed { id }` â†’ drop from index and cascade-delete the embedding row.
...
- `stale(now, max_age)` â€” ids whose `indexed_tick` is older than the SLA.
- `stale_as_of()` â€” the oldest `indexed_tick` across all tracked nodes.
- `freshness(now, sla)` â€” a `Freshness` verdict (`Fresh` or `Stale { as_of, stale_ids }`).
...
- `min_recall_at_k: f64` â€” floor for recall@k against exact-search ground truth.
- `max_p99_latency_ms: u64` â€” ceiling for tail latency.
...
- [`retrieval_core_hybrid_retrieval`](retrieval_core_hybrid_retrieval.md) â€” owns the immutable `Corpus`, BM25, embedding, and reranking logic.
- [`retrieval_core_acl`](retrieval_core_acl.md) â€” access-control lists for retrieval nodes.
- `retrieval_core_maintenance` (this module) â€” event-driven staleness tracking and SLO monitoring.
- [`retrieval_core_reembed`](retrieval_core_reembed.md) â€” plans and executes embedding migrations.

The `ReindexTrigger`s emitted by `IndexState` are consumed by the re-embedding and corpus-rebuild machinery in [`retrieval_core_reembed`](retrieval_core_reembed.md) and [`retrieval_core_hybrid_retrieval`](retrieval_core_hybrid_retrieval.md). The `Freshness` verdict and `stale_as_of` watermark feed into [`retrieval_advanced`](retrieval_advanced.md) (structured and federated retrieval) and the serving runtime in [`runtime_engine`](runtime_engine.md) so that responses can carry accurate freshness metadata.

Upstream, the module receives `SourceEvent`s from context ingestion in [`context_sources`](context_sources.md) and [`context_retrieval_routing`](context_retrieval_routing.md), as well as from runtime event streams in [`core_interaction`](core_interaction.md) and [`memory_management`](memory_management.md).

---

## Key Design Decisions

1. **Immutable corpus snapshot** â€” The `Corpus` itself is never mutated mid-query; maintenance produces triggers that drive a controlled rebuild. This keeps BM25 statistics (`df`, `avgdl`) consistent.
2. **Determinism** â€” Logical ticks are passed in, fingerprints use FNV-1a over bytes, and outputs are id-sorted. No wall clock, no RNG, no hash-map iteration order.
3. **Incremental, not batch** â€” Only nodes whose fingerprint changed are re-embedded; identical re-upserts are no-ops.
4. **Bounded memory** â€” `RecallLatencyMonitor` keeps fixed-size rolling windows for recall and latency samples.
5. **Freshness transparency** â€” Stale data is flagged with `stale_as_of` rather than silently served as current.
...
- Runtime integration: [`runtime_engine`](runtime_engine.md)
- Interaction/event streams: [`core_interaction`](core_interaction.md)
