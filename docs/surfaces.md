...
The **surfaces** module lives inside the `ainxt-runtimed` daemon and provides the *composition-root* turn handlers ("surfaces") that can be mounted on the served chat path (`POST /v1/chat`). Each surface is a [`TurnHandler`](runtime_engine.md) wrapper that adds a specific cross-cutting concernâ€”identity governance, multi-graph context grounding, digital-workforce role governance, or prompt-engineering optimizationâ€”before delegating to the real chat handler.
...
flowchart TB
    subgraph Surfaces["ainxt-runtimed surfaces"]
        direction TB
        GS[GovernedChatSurface<br/>Â§15 identity renew + Â§17/Â§19 admission]
...
1. Inspect or transform the [`Request`](core_interaction.md).
...
|---|---|---|---|
| Chat Identity | `chat_identity.rs` | Â§15 short-TTL JIT renew-and-re-attest + Â§17/Â§19 in-flight admission for every chat turn. | [surfaces_chat_identity.md](surfaces_chat_identity.md) |
...
- **[core_interaction](core_interaction.md):** Surfaces operate on `Request`/`Event`/`TurnSummary` protocol types.
- **[knowledge_retrieval](knowledge_retrieval.md):** `FabricGroundedChatSurface` consumes `MultiGraphFabric` and `compile_served_fabric` from the context/retrieval stack.
- **[governance_compliance](governance_compliance.md):** `GovernedChatSurface` drives `ControlPlane::authorize_dispatch` and `RunAuthorization`; `WorkforceSurface` integrates governance, CODEOWNERS, and Marketplace TOFU pinning.
- **[ai_engine/prompt_engineering](prompt_engineering.md):** The prompt optimizer surface bridges `ainxt-promptopt` into the live `Registry`.
- **[governance_compliance/workforce](workforce.md):** `WorkforceSurface` re-exports and composes the workforce factory, kernel, and controls.
