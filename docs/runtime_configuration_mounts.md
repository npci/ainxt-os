...
- **Governance & compliance**: [`admission`](admission.md) (`HarnessRegistry`, `HarnessRuntime`), [`governance`](governance.md) (`GovernanceState`), [`lifecycle`](lifecycle.md) (`RecordStore`), and [`responsible_ai`](responsible_ai.md) (`QualityCircuitBreaker`).
- **Security & identity**: [`security_config`](security_config.md) (`TokenVault`, `AeadCodec`, `Principal`), [`identity`](identity.md) (`ControlPlane`, `ControlPlaneRemediator`), and [`incident`](incident.md) (`IncidentRegister`).
- **Connectors**: [`connectors`](connectors.md) (`ConnectorRuntime`, `ConnectorRegistry`) and the HTTP-specific adapters in `ainxt-connector-http`.
- **AI engine**: [`answer_artifact`](answer_artifact.md) (`ArtifactRuntime`) and [`evaluation_testing/replay`](replay.md) (`SessionStore`, `ReExecutor`).
...
- [`admission`](admission.md) — harness registry, runtime, and governance.
- [`connectors`](connectors.md) — connector runtime and registry.
- [`answer_artifact`](answer_artifact.md) — artifact generation runtime.
- [`replay`](replay.md) — deterministic replay and drift detection.
- [`lifecycle`](lifecycle.md) — retention, legal hold, and DSAR record-tier erasure.
- [`responsible_ai`](responsible_ai.md) — quality circuit breaker and model-risk scoreboard.
