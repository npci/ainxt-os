...
The SDK is intentionally transport-agnostic: an [`InProcessTransport`] embeds the `SessionManager` and runs turns directly inside the caller's process, while a [`NetworkTransport`] (feature `http`) encodes the same contract over HTTP/SSE to a remote `ainxt-server`. Both transports yield the same [`ChatStream`] of typed `Event`s and the same [`Collected`] result semantics, so callers see a single surface regardless of deployment topology.
...
[`Client`] is the typed entry point. It is bound to one authenticated `Principal` and can run many turns across many sessions. It exposes:

- `chat(session, turn, input)` — convenience one-shot turn using the configured default `DataClass`.
...
- `run_harness_with_invoker_gated(...)` — same as above, but applies a `ComplianceGate` to every step result before chaining.
...
[`InProcessTransport`] implements this by calling `SessionManager::submit`. If the session cap is reached it returns [`ClientError::Backpressure`], which the caller can translate to an HTTP 503 or retry.
...
- `decode_event_frame` — decode one payload into an `Event` or the `[DONE]` sentinel.
...
- [`WireApprovalGate`] implements the runtime's `ApprovalGate` trait. It blocks on the coordinator and fails closed on timeout.
...
The harness bridge turns a declarative `HarnessManifest` into real execution:
...
- `run_harness_with_invoker` — `Tool`/`Skill` steps invoke their named capability through a [`CapabilityInvoker`], while `Llm` steps still stream through the engine. Autonomy (`none`/`assisted`/`autonomous`) and an `ApprovalResolver` enforce write approval.
- `run_harness_with_invoker_gated` — additionally applies a `ComplianceGate` to every step result, so untrusted tool/connector output is redacted before it is recorded or fed to the next step.
...
- [core_interaction.md](../core_infrastructure/core_interaction.md) — session, protocol, and event-log semantics.
- [ai_engine.md](../ai_engine/ai_engine.md) — the engine that executes turns behind the client.
- [governance_compliance.md](../governance_compliance/governance_compliance.md) — admission, approval, and harness governance.
...
Harness execution reports policy refusals (capability denied, data-class exceeded, approval rejected) inside `HarnessRunReport::outcome` rather than as a client error, because a refused harness is a successful SDK operation with a negative result.
