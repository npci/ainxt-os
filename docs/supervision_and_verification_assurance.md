...
- **Planning**: The artifact's `edit_rung` is checked against the node contract's `edit_ladder_floor` in [`ProgramState`](planning_program_execution_program_execution_state.md). A below-floor artifact is refused even with a green three-way proof.
...
- [planning_program_execution_program_execution_state](planning_program_execution_program_execution_state.md) — durable node verdicts and edit-rung floors.
