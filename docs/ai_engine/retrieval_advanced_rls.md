...
`RlsSession` is the `SET LOCAL` half of RLS. It holds a sorted map of bound setting names to values captured from the caller's [`Principal`](../core_infrastructure/security_config_identity.md#principal) or supplied by the composition root.
...
The typical caller is the runtime engine ([`runtime_engine`](../pipeline_runtime/runtime_engine.md)), which:

1. Receives a turn request carrying an authenticated [`Principal`](../core_infrastructure/security_config_identity.md#principal).
...
- [`core_interaction`](../core_infrastructure/core_interaction.md) — the composition root writes `BreakGlassAudit` records to the event log for compliance.
...
- [`security_config_identity`](../core_infrastructure/security_config_identity.md) supplies the `Principal` type and OBO claims (`user_id`, `department`, granted capabilities).
...
- [`security_config_identity`](../core_infrastructure/security_config_identity.md) — `Principal`, OBO claims, and capability model.
- [`core_interaction`](../core_infrastructure/core_interaction.md) — event logging and audit sinks.
- [`runtime_engine`](../pipeline_runtime/runtime_engine.md) — the engine that binds principals and invokes retrieval.
