...
1. **Real-runtime scenario execution**  
   Map each [`Scenario`](../scenario_service/scenario_service.md) from the generated matrix into a real turn of the [`Engine`](../pipeline_runtime/runtime_engine.md) and capture the resulting [`Observation`](../scenario_service/scenario_service.md).
...
A [`Provider`](../pipeline_runtime/runtime_engine.md) implementation that is **always retryable-failing**. It is registered as the router's primary so that every conformance turn is forced through the failover path to `ConformanceProvider`.
...
The deterministic backup provider. It parses the scenario directive embedded in the prompt (via [`ainxt_scenario::matrix::parse_directive`](../scenario_service/scenario_service.md)) and returns an event stream that exercises the invariant under test.
...
A side-effecting [`Tool`](../tools_cli/tools_cli.md) that records every execution in a shared `Arc<Mutex<Vec<String>>>`.
...
A structured side-effecting [`Tool`](../tools_cli/tools_cli.md) with a JSON schema requiring `"account": String`. It is used to verify that the runtime rejects malformed tool-call JSON before execution.
...
Implements [`Target`](../scenario_service/scenario_service.md) from the scenario harness. It owns a fully assembled [`Engine`](../pipeline_runtime/runtime_engine.md) and a Tokio runtime, and maps each scenario into a real turn.
...
- **Scenario service** ([`scenario_service`](../scenario_service/scenario_service.md)) proves the oracles and matrix generation work.
...
| Function | Purpose |
|----------|---------|
| `run_matrix()` | Runs the full generated matrix (`ainxt_scenario::matrix::matrix_suite()`) and returns a [`Report`](../scenario_service/scenario_service.md). |
| `run_pairwise_matrix()` | Runs the pairwise-generated corpus (`pairwise_matrix_suite()`) and returns a [`Report`](../scenario_service/scenario_service.md). |
...
- [`scenario_service`](../scenario_service/scenario_service.md) — scenario definitions, oracles, matrix generation, and the `Runner` harness.
- [`runtime_engine`](../pipeline_runtime/runtime_engine.md) — the `Engine`, `ModelRouter`, `RbacAuthorizer`, and cancellation semantics.
- [`tools_cli`](../tools_cli/tools_cli.md) — `ToolRuntime`, `Tool`, `InMemoryLedger`, and structured tool schemas.
- [`governance_compliance`](../governance_compliance/governance_compliance.md) — `StrongRedactor` and compliance redaction.
- [`injection_service`](../injection_service/injection_service.md) — injection detection and taint-gating configuration.
- [`core_interaction`](../core_infrastructure/core_interaction.md) — `Event`, `Request`, and protocol envelopes.
- [`security_config_identity`](../core_infrastructure/security_config_identity.md) — `Principal` and `DataClass`.
