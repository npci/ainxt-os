...
A `QualityDimension` is a named, deterministic heuristic that maps an `EvaluableAnswer` to a 0–100 `DimensionScore` with a rationale. The standard dimensions are:

| Dimension | Purpose | Key Inputs |
|-----------|---------|------------|
| `Completeness` | Coverage of required points from the question | `required_points` |
| `FormatValidity` | Conformance to declared output format (`Prose`, `Markdown`, `BulletList`, `Table`, `Json`) | `expected_format` |
| `VerbosityFit` | Length inside target word band | `target_len` (`LengthBand`) |
| `CitationPresence` | Claims cited with in-range markers when sources exist | `sources`, citation markers `[n]` |
| `Groundedness` | Fraction of content words supported by sources | `sources`, answer text |
| `ToneConsistency` | Professional tone; penalties for hedging, apologies, shouting | `data_class` (stricter for regulated) |
...
`QualityAssessor` runs a fixed set of dimensions with configurable `QualityWeights` and produces a `QualityProfile`. Weights are non-negative and default to uniform. The profile exposes per-dimension scores and a weighted overall score.

### 3. Drift Detection (`lib.rs`)

`detect_drift` compares an ordered baseline window of profiles against a recent window using a two-sample change-point test. A regression must exceed both an absolute `drop_margin` and a `z_threshold` standard-error threshold. The result is one of:
...
- `DimensionJudge<F>` — drives [`ainxt_eval::QualityJudge`](evaluation_testing.md) from a single dimension.
- `ProfileJudge<F>` — drives the gate from the aggregate `QualityProfile`.
...
`SampledDriftMonitor` ingests live quality scores, samples every Nth observation deterministically, and feeds a tabular `Cusum` detector. On a downward change-point it invokes a `DriftResponder` to open a ticket and optionally roll back.
...
The monitor also supports a **provider silent-update tripwire** via `provider_silent_update`, which re-scores a frozen test set and uses a Welch t-test plus a control-plane-change flag to isolate unexplained provider model shifts from self-inflicted changes.

### 6. Online Release Controller (`controller.rs`)

`OnlineReleaseController` composes three existing pieces into a live-drivable safety loop:
...
The controller consumes a `LiveTurnFeed` of `ObservedTurn` observations (`served_ref`, `quality`) and uses production seams for the pointer flip, human notification, and drift response. It is fully deterministic and replayable via the in-memory test doubles.

### 7. Live-Traffic Feed (`feed.rs`)

`LiveTurnFeed` is the seam between the served path and the controller. `ReplayFeed` provides a FIFO implementation that can be preloaded for offline replay or pushed by a served hook stand-in. The production feed is driven by the runtime daemon and is infra-gated.
...
stateDiagram-v2
    [*] --> Canarying
    Canarying --> Canarying : candidate turn#58; record, decide, hold
    Canarying --> Promoted : AvDecision#58;#58;Promote
    Canarying --> RolledBack : AvDecision#58;#58;Rollback
...
- [`runtime_engine`](../pipeline_runtime/runtime_engine.md) / [`server_serving`](../pipeline_runtime/server_serving.md) — the runtime daemon wires the production `LiveTurnFeed` onto the served turn handler and supplies the real `PointerController`, `Notifier`, and `DriftResponder`.
...
- [runtime_engine](../pipeline_runtime/runtime_engine.md) — turn execution and runtime surfaces.
- [server_serving](../pipeline_runtime/server_serving.md) — serving infrastructure that hosts the live feed.
