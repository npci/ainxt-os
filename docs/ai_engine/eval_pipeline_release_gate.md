...
| `score_and_gate` | Paired scoring loop, CUPED adjustment, panel aggregation, and statistical gate invocation. | [eval_judging.md](eval_judging.md), eval_pipeline_stats.md |
| `statistical_gate` | Per-cell non-inferiority testing with FDR / Holm correction. | eval_pipeline_stats.md |
| `meta_gate_eval_set` | Validates pre-registration and checks statistical power. | eval_pipeline_manifest.md |
| `SealedCorpusStore` / `SealedManifest` | Loads the gold corpus and verifies its Merkle content commitment. | eval_pipeline_integrity.md |
...
| `scan_contamination` | Detects candidate memorization of eval cases via n-gram and embedding overlap. | eval_pipeline_integrity.md |
| `Tripwire` | Overfit detector comparing visible-case and sealed tripwire-case performance. | eval_pipeline_integrity.md |
...
- eval_pipeline_stats.md — `statistical_gate`, CUPED, and cell-level testing.
- eval_pipeline_integrity.md — sealed corpus, contamination scanning, and tripwire.
