...
|--------|-------------------|
| [knowledge_retrieval_context_retrieval_routing](context_retrieval_routing.md) | Memory hits are fed into the Context Optimizer / `MultiGraphFabric` as layer 12 of the fabric. |
| [runtime_engine](../pipeline_runtime/runtime_engine.md) | `Engine` and turn surfaces call `read_for_turn` during turn construction. |
...
| evaluation_testing_replay | Replay infrastructure uses `TurnLineage::refs` with `InMemoryStore::resolve` for deterministic re-execution. |
...
- [knowledge_retrieval_context_retrieval_routing](context_retrieval_routing.md) — Context Optimizer and fabric routing where memory is consumed as layer 12.
- [runtime_engine](../pipeline_runtime/runtime_engine.md) — turn execution engine that invokes the fabric read path.
- evaluation_testing_replay — deterministic replay using `TurnLineage`.
