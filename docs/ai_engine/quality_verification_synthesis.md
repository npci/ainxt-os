...
- [`Source`] — A retrieved source with `id`, `text`, `DataClass`, and optional `authority`/`timestamp` for conflict arbitration.
...
- **Conversation / Runtime** — [`ainxt-convo`](../core_infrastructure/surface_conversation.md) and [`ainxt-runtimed`](../pipeline_runtime/runtime_configuration.md) call the live verification gates before streaming an answer to the user.
...
Set to `DataClass::Confidential`. Any answer grounded on a source at or above this floor triggers the hard numeric re-derivation block by default in [`LedgerAnswerGate`].
...
- [surface_conversation](../core_infrastructure/surface_conversation.md) — Conversation runtime that calls synthesis gates.
- [runtime_configuration](../pipeline_runtime/runtime_configuration.md) — Runtime configuration and served surfaces.
...
- ainxt-types — `DataClass` and other shared types.
