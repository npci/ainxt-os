...
`AccessContext` is the **caller-side** read context. It is built from a [`security_config_identity`](../core_infrastructure/security_config_identity.md) `Principal` and carries every axis a `NodeAcl` can gate on:
...
- `AccessContext::from_principal(&Principal)` — the live served grounding path builder. Called every turn by the conversation layer (e.g., [`surface_conversation_intelligence`](../core_infrastructure/surface_conversation_intelligence.md)) when assembling grounding.
...
|----------------|-----------------|------|
| [`security_config_identity`](../core_infrastructure/security_config_identity.md) | `Principal`, `DataClass` | Source of caller claims and scalar clearance |
| [`core_interaction`](../core_infrastructure/core_interaction.md) | Session / turn resolution | Builds the live `Principal` from JWT/OBO tokens |
...
| [`surface_conversation_intelligence`](../core_infrastructure/surface_conversation_intelligence.md) | Calls `AccessContext::from_principal` each turn |
...
- Dynamic connector-scope authorization — see [`connectors_runtime`](../core_infrastructure/connectors_runtime.md) and [`security_config_identity`](../core_infrastructure/security_config_identity.md).
...
- [`security_config_identity`](../core_infrastructure/security_config_identity.md) — `Principal`, `DataClass`, and identity claims.
- [`core_interaction`](../core_infrastructure/core_interaction.md) — session and turn-level context resolution.
