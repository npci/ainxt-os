...
This module is a child of [`safety_guardrails`](safety_guardrails.md) and a sibling of [`safety_guardrails_rails`](safety_guardrails_rails.md). The related service crate [`injection_service`](../injection_service/injection_service.md) exposes a standalone HTTP scanning service built on the same primitives.
...
| [`injection_service`](../injection_service/injection_service.md) | Standalone HTTP service crate that wraps the same detector, judge pipeline, and policy layers for external scanning workloads. |
| [`core_interaction`](../core_infrastructure/core_interaction.md) | `ainxt-protocol` defines `Request.untrusted_tainted`, which the runtime reads to gate tool dispatch. |
| [`application_runtime`](../core_infrastructure/application_runtime.md) | `ainxt-runtime`/`ainxt-runtimed` consume taint flags and call `gate_tool_on_taint_for_turn` / `guard_egress_for_turn`. |
| [`surface_conversation`](../core_infrastructure/surface_conversation.md) | `ainxt-chat`/`ainxt-convo` implement the production scan+fence+taint path for served chat surfaces. |
| [`knowledge_retrieval`](knowledge_retrieval.md) | `ainxt-context` applies `wrap_untrusted` during context compilation. |
| [`security_config`](../core_infrastructure/security_config.md) | `ainxt-config` loads runtime configuration including `InjectionDefenseConfig`. |
