...
- **Security / ACL / RLS**: Re-exports `AccessContext`, `NodeAcl`, `RowFilter`, `RlsSession`, `RlsPolicy` from `ainxt-retrieval`. See [security_config](../core_infrastructure/security_config.md).
...
- **Event log**: `TurnEventRecord` is the payload handed to `ainxt_eventlog`. See [core_interaction](../core_infrastructure/core_interaction.md).
