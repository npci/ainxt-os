...
The crate provides reusable primitives (`JudgePanel`, `StuckDetector`, `Reviewer`, `JudgeLoop`) that are consumed by the production self-heal pipeline in [`pipeline_orchestration`](../pipeline_runtime/pipeline_orchestration.md) and by evaluation harnesses in [`evaluation_testing`](evaluation_testing.md).
...
sequenceDiagram
    ...
    Note over JudgePanel: Drops self_summary#59; only candidate crosses
...
The production pipeline encodes the same idea more strongly in the type system via `PipelineOutcome::Complete` and a private `CommitApproval` seal; see [`pipeline_orchestration`](../pipeline_runtime/pipeline_orchestration.md).
