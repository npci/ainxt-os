...
The `llm_providers_vendor_adapters` module provides vendor-specific HTTP/SSE adapters that normalize streaming responses from third-party large language model (LLM) endpoints into the system-wide [`Event`](core_interaction.md) protocol. It is the outermost edge of the [LLM providers](llm_providers.md) layer and implements the multi-model policy: every runtime feature must work irrespective of whether the underlying model is served by an OpenAI-schema endpoint, Anthropic, or Google Gemini.
...
3. **Data-class eligibility** — Declare which [data classifications](security_config.md) each endpoint is allowed to serve, enforced by the router.
...
The module does **not** handle routing, retries, load balancing, caching, or model selection. Those concerns live in the [runtime engine](runtime_engine.md), [serving infrastructure](server_serving.md), and [prompt core](prompt_core.md) modules.
...
- **Model-as-identity**: Each provider's `Provider::id` returns the configured model name. This lets the [runtime engine](runtime_engine.md) route by model without maintaining a separate alias table.
...
|------------|--------|------|
| `ainxt_protocol::Event` | [core_interaction](core_interaction.md) | Canonical streaming event enum (`TextDelta`, `Usage`, `Done`, `Error`). |
| `ainxt_runtime::provider::Provider` | [runtime_engine](runtime_engine.md) | Trait that all adapters implement. |
| `ainxt_types::DataClass` | [security_config](security_config.md) | Data classification used for eligibility checks. |
...
Upstream callers such as the [runtime engine](runtime_engine.md)'s `ModelRouter` hold a collection of `Provider` trait objects. When a turn is ready for inference, the router selects an eligible provider (based on model id and data class), calls `stream(prompt)`, and consumes the resulting `Event` stream. The prompt engine and prompt optimizer produce the prompt text; they do not interact with vendor specifics.
...
- [runtime_engine](runtime_engine.md) — Model routing, inference execution, and data-class enforcement.
- [core_interaction](core_interaction.md) — `Event` protocol and turn lifecycle.
- [security_config](security_config.md) — `DataClass` and eligibility policies.
