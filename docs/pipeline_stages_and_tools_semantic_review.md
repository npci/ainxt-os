...
- **Confidence input**: The coverage overlap feeds into the [classification and risk](pipeline_stages_and_tools_classification_and_risk.md) confidence score; coupling advisories are informational only.
...
- **Downstream consumers**: The [commit gate](pipeline_stages_and_tools_commit_gate.md) consumes `architecture_violations` as a hard blocker. The [classification and risk](pipeline_stages_and_tools_classification_and_risk.md) module folds `coverage` and regression advisories into the confidence score.
- **Upstream integration**: `EditEngine::with_semantic_review` wires a `SemanticGateConfig` into the engine so that every live edit turn automatically runs semantic review. The [self-healing](pipeline_stages_and_tools_self_healing.md) loop invokes `analyze_semantic_gate` on each candidate file set it produces.
...
- [`pipeline_stages_and_tools_classification_and_risk`](pipeline_stages_and_tools_classification_and_risk.md) — Consumes coverage and regression advisories for confidence scoring.
- [`pipeline_stages_and_tools_self_healing`](pipeline_stages_and_tools_self_healing.md) — Invokes semantic review during self-heal rounds.
