...
The production adapter over [`ainxt_memory::store::InMemoryStore`](memory_management.md). It:
...
- Deletes per-item via [`MemoryStore::delete_as`](memory_management.md), attributed to the subject.
- Uses `unanchored_tick = now` for items with no `effective_from`, failing toward preservation.

### `SessionReplayTier`

The production adapter over [`ainxt_replay::SessionStore`](evaluation_testing.md). It:
...
- Never deletes a [`Turn`](evaluation_testing.md); instead it clears event content through `SessionRecording::erase_turn_content`, leaving turn ids and tree structure as audit-visible tombstones.
...
| `ainxt_memory` | [`memory_management`](memory_management.md) | [`InMemoryStore`](memory_management.md) adapter via [`MemoryFabricTier`] |
| `ainxt_replay` | [`evaluation_testing`](evaluation_testing.md) | [`SessionStore`](evaluation_testing.md) adapter via [`SessionReplayTier`] |
| `ainxt_types` | [`security_config_identity`](security_config_identity.md) | [`DataClass`](security_config_identity.md), [`Principal`](security_config_identity.md) |
...
For deterministic tests, [`MapTier`] provides a fully in-memory `ErasableTier`; for integration tests, [`SessionReplayTier`] is exercised against a real [`InMemorySessionStore`](evaluation_testing.md).
