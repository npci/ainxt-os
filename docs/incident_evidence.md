...
- **security_config / ainxt-types**: supplies the `Principal` type and capability model. See [`security_config`](security_config.md).
- **server_serving / ainxt-server**: the `RegFiEvidenceRequest` route and related HTTP handlers wire `evidentiary_export_for` into the public API. See [`server_serving_core`](server_serving_core.md).
- **pipeline_runtime / ainxt-runtimed**: injects live runtime version, NTP attestation, and control-plane SHA when producing exports in production. See [`runtime_configuration`](runtime_configuration.md).
