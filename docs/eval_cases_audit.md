...
A minimal seam that keeps the eval core decoupled from any specific logging backend. The production implementation is provided by the [`eventlog`](eventlog.md) crate (tamper-evident, chain-hashed JSONL). Test code can implement `EventSink` with an in-memory vector.
...
| [`eventlog`](eventlog.md) | Provides the tamper-evident `JsonlEventLog` that backs the production `EventSink` implementation. |
| [`security_config_identity`](security_config_identity.md) | Defines `DataClass` and its regulated/PII classification used by `route_judge`. |
...
- [`security_config_identity.md`](security_config_identity.md)
