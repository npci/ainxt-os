...
Power is computed via [`is_powered`](eval_cases_judging.md) from the [`eval_judging`](eval_judging.md) statistics module. If the sample SD is ≤ 0, the effect is considered trivially detectable.
...
- [`eval_cases_judging`](eval_cases_judging.md) — for `is_powered`, the statistical power calculation used by the meta-gate.
...
- [`eval_judging`](eval_cases_judging.md) — provides the statistical utilities (`is_powered`, power analysis) used by the meta-gate.
