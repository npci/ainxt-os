...
1. **Which state transitions are legal?** A settlement moves `Draft â†’ Reserved â†’ Committed`, with `Compensated`, `Failed`, and `InDoubt` as off-ramps. Illegal transitions are always rejected, never silently ignored.
2. **Did we already do this?** Every settlement is keyed on an `idempotency_key`. Committing the same key twice returns the first outcome and applies no second effect â€” the exactly-once guarantee.
...
The payments module does **not** grant agents the ability to move money. That apex guarantee lives in the capability-registry / effect-class layer (see [core_infrastructure](core_infrastructure.md) and [pipeline_runtime](pipeline_runtime.md)). Instead, `ainxt-payments` provides the correctness core that authorized payment-system code is built on.
...
- **Egress time**: `boundary` is called by the connector/http egress path ([core_infrastructure / connectors_http](connectors_http.md)) before any bytes leave. A match triggers identity revocation ([security_config_identity](security_config_identity.md)) and incident filing ([governance_compliance / incident](incident.md)).
- **Adjacent dispatch time**: `mandate` composes with the OBO layers provided by [security_config_identity](security_config_identity.md) as a fourth gate.
- **Settlement time**: `settlement` is used by the runtime engine ([pipeline_runtime / runtime_engine](runtime_engine.md)) and server ([pipeline_runtime / server_serving_core](server_serving_core.md)) to move reserved funds exactly once.
