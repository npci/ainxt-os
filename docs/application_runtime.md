...
- **Consumers**:
  - [`runtime_engine`](runtime_engine.md) executes the turns produced by the conversation surface.
  - [`server_serving`](server_serving.md) exposes the chat surface over HTTP and wires runtime configuration.
  - [`tools_cli`](tools_cli.md) invokes plugins through the same `PluginHost` seam.
- **Peers**:
  - [`security_config`](security_config.md) supplies cryptographic primitives for plugin supply-chain verification.
  - [`prompt_engineering`](prompt_engineering.md) provides layered prompt assembly consumed by the skill and conversation layers.
  - [`knowledge_retrieval`](knowledge_retrieval.md) supplies grounded retrieval and citation assembly.
  - [`safety_guardrails`](safety_guardrails.md) provides output-side rails and injection defense.
