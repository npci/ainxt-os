# `context_sources_artifacts` â€” Multimodal Artifact Ingestion & Retrieval
...
1. **Artifact modelling** â€” Represents multimodal artifacts (`Artifact`), their derived vector rows
   (`DerivedEmbedding`), and the in-house/cloud models that may embed them (`ArtifactModel`).
2. **Data-class model routing** â€” Selects an eligible embedding model for an artifact based on its
   `modality` and `data_class`. Regulated/PII artifacts are refused unless an in-house model exists.
3. **Pluggable embedding** â€” Defines the `ArtifactEmbedder` seam so infrastructure can inject the
   real vision/ASR model without coupling the routing/RBAC logic to it.
4. **Namespace-isolated, RBAC-aware search** â€” `ArtifactStore::search` filters results **pre-rank**
...
5. **Erasure cascade** â€” `ArtifactStore::erasure_cascade` returns an `ErasurePlan` that covers both
...
| `ingest_artifact` | End-to-end ingestion pipeline: route â†’ embed â†’ index atomically, with fail-visible errors. |
...
- `Artifact` is the central domain object. It carries a `data_class` from
  [`security_config`](security_config.md) and an optional `NodeAcl` from
...
- **Upstream**: `context_sources_artifacts` depends on [`security_config`](security_config.md) for
...
- [`context_sources`](context_sources.md) â€” parent module covering all context-source pipelines.
- [`context_sources_fabric_extraction`](context_sources_fabric_extraction.md) â€” text and code
  extraction sibling.
- [`context_retrieval_routing`](context_retrieval_routing.md) â€” context optimizer and hybrid
  retriever that consumes artifact output.
- [`retrieval_core`](retrieval_core.md) â€” shared retrieval primitives (`NodeAcl`, `AccessContext`,
  embedding/reranking interfaces).
- [`security_config`](security_config.md) â€” `DataClass` definitions and clearance model.
- [`ai_engine`](ai_engine.md) â€” top-level AI engine documentation.
