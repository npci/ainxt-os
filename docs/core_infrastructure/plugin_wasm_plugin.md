...
- **[`tools_cli`](../tools_cli/tools_cli.md)** — `ainxt-tools` exposes `PluginCapability`, bridging tool dispatch into the plugin host.
- **[`runtime_engine`](../pipeline_runtime/runtime_engine.md)** — `ainxt-runtimed` registers `ApprovedPlugin` instances and mixes `NativeHost` and `WasmPluginHost` behind a single trait object.
...
- **[`tools_cli`](../tools_cli/tools_cli.md)** — tool runtime that bridges `PluginCapability` to the plugin host.
- **[`runtime_engine`](../pipeline_runtime/runtime_engine.md)** — served runtime that registers and dispatches approved plugins.
