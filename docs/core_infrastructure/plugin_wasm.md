...
`plugin_wasm` sits inside the larger [`application_runtime`](application_runtime.md) subsystem and is consumed by [`skill_execution`](skill_execution.md) (which dispatches to `WasmSkillExecutor` / `NativeSkillExecutor`) and by the tool runtime in [`tools_cli`](../tools_cli/tools_cli.md). It also depends on [`security_config`](security_config.md) for identity and cryptographic primitives used during plugin supply-chain verification.
...
sequenceDiagram
    ...
    alt verification fails
        ...
        WH->>WH: resolve requested ∩ granted capabilities
...
- **Consumer**: [`runtime_engine`](../pipeline_runtime/runtime_engine.md) carries `ApprovedPlugin` and the served plugin registration path; it passes the same `PluginHost` trait object to the tool runtime.
