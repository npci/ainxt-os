...
stateDiagram-v2
    ...
    Connecting --> AuthRequired : McpError#58;#58;AuthRequired
...
|----------|------------|-------|
| `ainxt-tools` | [`tools_cli`](../tools_cli/tools_cli.md) | MCP tools, once discovered, flow through the same OBO dispatch path as native tools. |
| `ainxt-runtimed` | [`runtime_engine`](../pipeline_runtime/runtime_engine.md) | `McpConfig`, `McpServerConfigEntry`, and `McpAdminHandle` wire MCP servers into runtime surfaces. |
| `ainxt-server` | [`server_serving`](../pipeline_runtime/server_serving.md) | `McpApproveRequest`, `ClearQuarantineRequest`, and admin handles expose pinning/approval to operators. |
...
- [`tools_cli`](../tools_cli/tools_cli.md) — Tool dispatch, OBO policy, idempotency, and egress DLP.
- [`runtime_engine`](../pipeline_runtime/runtime_engine.md) — Runtime engine that wires MCP surfaces into serving.
- [`server_serving`](../pipeline_runtime/server_serving.md) — Server surfaces and admin endpoints for MCP approval.
