...
* **Runtime**: The providers implement the `Provider` trait defined in `ainxt-runtime` (see [runtime engine](ai_engine/runtime_engine.md)). The runtime's `Engine` and `ModelRouter` select a provider based on model ID and data-class eligibility.
...
* **Classification**: `ProviderLabelModel` is consumed by `ainxt_convo::ModelIntentClassifier` (see [surface conversation intelligence](ai_engine/surface_conversation_intelligence.md)) to run Stage-2 intent classification on a real model.
