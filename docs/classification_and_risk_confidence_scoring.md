# Classification and Risk â€” Confidence Scoring
...
The module returns both a numeric score (`0â€“100`) and a full human-readable breakdown of every deduction, so reviewers and regulators can see *why* a score is what it is.

---

## Role in the System

This module sits inside the [pipeline_orchestration](pipeline_orchestration.md) â†’ [classification_and_risk](classification_and_risk.md) subsystem. Its relationship to sibling modules is:
...
| `perf_regression_penalty` | Performance stage | `0â€“25` penalty derived from benchmark regression and complexity growth. |
...
| Field | Meaning |
|-------|---------|
| `score` | Integer `0â€“100`. Higher is less residual risk. |
...
1. **SAST medium/low findings** â€” capped at `40`.
2. **Performance regression** â€” capped at `25`.
3. **Architecture boundary violations** â€” `15` points each.
4. **Regression risk** â€” `30 Ã— (1 - blast_radius_test_coverage)`.
5. **Unresolved review findings** â€” severity-weighted, capped at `30`.
6. **Skipped stages** â€” `5` points each.
7. **Edit-engine rung** â€” penalty depends on the least-trusted rung used.
...
flowchart LR
    ...
    Cov["Regression risk<br/>30 Ã— (1 - coverage)"] --> Review
...
The performance stage produces a pre-computed `regression_penalty` in the range `0â€“25`. This module simply clamps and applies it. See [performance](performance.md) for how benchmark slowdown and complexity growth are translated into the penalty.
...
- `ainxt_judge` â€” `ReviewFinding`, `ReviewSeverity` (see [quality_verification_judge](quality_verification_judge.md)).
- `ainxt_semantic` â€” `Rung` from the edit-engine ladder (see [edit_semantic](edit_semantic.md)).
- `crate::sast` â€” `SastFinding`, `Severity` (see [pipeline_stages_and_tools](pipeline_stages_and_tools.md)).
...
1. **Hard gates** â€” Phase-A failures, critical/high SAST findings, and architecture violations block regardless of score.
2. **Confidence score** â€” Determines the approval band once hard gates pass.
3. **Judge verdict** â€” Required for Tier 2+ edits; must come from an independent panel.

The `GatePolicy` defines the thresholds:

- `auto_complete_threshold` â€” score at/above which the edit auto-completes (if Judge approves when required).
- `review_threshold` â€” score at/above which the edit completes but is flagged for post-commit spot-audit.
- `trivial_auto_approve_floor` â€” special fast path for `Trivial` tier edits that clear all hard gates.

See [classification_and_risk_commit_gate](classification_and_risk_commit_gate.md) for the full gate logic.

```mermaid
flowchart TD
    A[All deterministic hard gates passed?] -->|No| B[Block]
    A -->|Yes| C[Compute Confidence Score]
    C --> D{Score â‰¥ auto_complete_threshold?}
    D -->|Yes| E{Judge required & approved?}
    E -->|Yes| F[Auto-complete]
    E -->|No| G[Fall through to review band]
    D -->|No| H{Score â‰¥ review_threshold?}
...
- [classification_and_risk](classification_and_risk.md) â€” parent module overview.
- [classification_and_risk_edit_classification](classification_and_risk_edit_classification.md) â€” how edits are classified and blast radius is computed.
- [classification_and_risk_risk_tiering](classification_and_risk_risk_tiering.md) â€” how risk tiers are assigned.
- [classification_and_risk_commit_gate](classification_and_risk_commit_gate.md) â€” how the confidence score is combined with Judge verdicts and hard gates.
- [pipeline_stages_and_tools](pipeline_stages_and_tools.md) â€” SAST, architecture, and review stages that feed the scorer.
- [performance](performance.md) â€” performance regression penalty computation.
- [edit_semantic](edit_semantic.md) â€” the semantic edit ladder and `Rung` penalties.
- [quality_verification_judge](quality_verification_judge.md) â€” the independent Judge panel and review findings.
