...
* [core_engine.md](core_engine.md) â€” the underlying `Engine`, `TurnHandler`, and dispatch primitives that `runtime_configuration` assembles surfaces over.
* [surfaces.md](surfaces.md) â€” chat, workforce, and prompt-optimizer surface builders consumed by `runtime_configuration`.
* [program_governance_and_execution.md](program_governance_and_execution.md) â€” program/team execution and governance wiring.
* [server_serving.md](server_serving.md) â€” the HTTP transport and serving infrastructure that receives `AssembledFull`.
* [knowledge_retrieval.md](knowledge_retrieval.md) â€” the retrieval and context-fabric modules fed by `KbConfig`.
* [memory_management.md](memory_management.md) â€” the durable memory store whose backend is threaded through `Assembled`.
* [connectors.md](connectors.md) â€” connector runtime and OAuth gateway mounted by `runtime_configuration_mounts`.
* [governance_compliance.md](governance_compliance.md) â€” admission, identity, incident, lifecycle, and responsible-AI organs wired into `AssembledFull`.
