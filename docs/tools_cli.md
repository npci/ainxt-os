# tools_cli â€” Headless CLI, Client SDK, Tool Runtime & Surface Profiles
...
The module is deliberately **non-interactive** â€” the headless CLI (`ainxt-cli`) is built for SSH boxes, air-gapped hosts, and CI, while richer surfaces are handled by the desktop application. All heavy lifting (model routing, compliance, RBAC, session management) is delegated to the [core_infrastructure](core_infrastructure.md), [ai_engine](ai_engine.md), [pipeline_runtime](pipeline_runtime.md), and [governance_compliance](governance_compliance.md) modules; this module only provides the thin, deterministic wrappers that connect human or script input to those subsystems.
...
- **Core interaction** â€” uses [core_interaction](core_interaction.md) (`ainxt-protocol`, `ainxt-session`) for the wire contract, session lifecycle, and event streaming.
- **Security & config** â€” uses [security_config](security_config.md) (`ainxt-types`, `ainxt-config`) for principals, data classes, and layered configuration loading.
- **AI engine** â€” chat turns and harness steps are executed by the [ai_engine](ai_engine.md) through the runtime engine; compliance redaction and guardrails run inside the engine, not in the CLI.
- **Governance** â€” harness linting, pre-receive gating, and publish workflows delegate to [governance_compliance](governance_compliance.md) (`ainxt-admission`, `ainxt-governance`).
- **Application runtime** â€” surface profiles configure [application_runtime](application_runtime.md) surfaces; the integration script exercises the [pipeline_runtime](pipeline_runtime.md) server layer.
- **Payments** â€” the tool runtime adopts `ainxt_payments::boundary::PaymentEffectClass` directly from [governance_compliance](governance_compliance.md) so payment-initiating tools are structurally non-dispatchable.
...
2. **Offline provider.** `ainxt-cli` ships an `OfflineProvider` so it works with no network or model configured â€” essential for air-gap smoke tests and CI.
...
- `ainxt run [PROMPT]` â€” run a single chat turn against the embedded offline runtime.
- `ainxt harness <lint|publish|dev|test> <MANIFEST.json>` â€” author, validate, and locally execute declarative harnesses.
- `ainxt sdk emit <python|typescript>` / `ainxt sdk contract` â€” emit generated SDK bindings or the machine-readable contract descriptor.
- `scripts/test_multi_turn_context.py` â€” integration smoke test for conversation history and session isolation.

## See Also

- [Headless CLI](tools_cli_headless_cli.md) â€” `ainxt-cli` parsing, orchestration, and subcommands.
- [Client SDK](tools_cli_client_sdk.md) â€” `ainxt-client` wire contract, transport seam, harness bridge, and SDK codegen.
- [Tool Runtime](tools_cli_tool_runtime.md) â€” `ainxt-tools` capability registry, hooks, ledger, and reconciler.
- [Surface Profiles](tools_cli_surface_profiles.md) â€” `ainxt-profile` schema and layered resolution.
- [Integration Tests](tools_cli_integration_tests.md) â€” multi-turn context retention smoke tests.
