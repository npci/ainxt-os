...
flowchart LR
    ...
    ES -->|production| eventlog["[core_interaction](core_infrastructure.md#core_interaction) Event Log"]
```
...
- **[core_interaction](core_infrastructure.md#core_interaction)** — The production `EventSink` is typically backed by the hash-chained event log (`ainxt-eventlog`).
