...
This mechanism is the serving-side implementation of the "big red button" described in the governance and identity modules. See [`identity`](identity.md) and [`incident`](incident.md) for the authority and kill-switch lifecycle.
...
| `AgentWorkloadCredential` / `run_id` | [`identity`](identity.md) | Kill-switch correlation |
...
- [`identity`](identity.md) — identity plane, `AgentWorkloadCredential`, and kill-switch semantics
- [`incident`](incident.md) — incident response and kill-switch lifecycle
