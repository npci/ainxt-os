...
- **[health](health.md)** — `AutoscaleCadence` mirrors the cadence pattern used by `HealthCadence` and `AttestationRefresher`. The health subsystem supplies the fleet state (bin health, drain/replace events) that influences which bins are available for placement.
...
- **[health](health.md)** — fleet health, drain/replace events, and cadence patterns.
