...
`SteerabilityConfig` is resolved through the same layered TOML merge as every other config domain (see [`security_config`](security_config.md) / `ainxt_config::Loader`). When configured, [`prompt_core_assembly`](prompt_core_assembly.md) and the served prompt builders use `is_eligible` to drop model families that fail the instruction-following bar. An empty config keeps the gate inactive and preserves legacy behavior.
...
- [`security_config`](security_config.md) — layered configuration loading via `ainxt_config::Loader`.
