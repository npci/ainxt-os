...
See [security_config_identity.md](security_config_identity.md) for the `Principal` and identity primitives that roles ultimately resolve against.
...
- **`pipeline_runtime`** (`ainxt-runtimed/src/program_exec.rs`) drives served team runs using the fan-out/cancellable wrappers. See [pipeline_runtime.md](pipeline_runtime.md).
- **`ai_engine`** planners such as `ainxt-planner::qos::ElasticFanoutPolicy` compute the `fan_out_ceiling` fed into `run_team_fanout`. See [ai_engine.md](ai_engine.md).
- **`core_infrastructure`** provides `ModelTier` (from `ainxt-types`) and the identity, security, and runtime config layers that roles and capabilities resolve against. See [core_infrastructure.md](core_infrastructure.md).
...
- [pipeline_runtime.md](pipeline_runtime.md) — Live runtime that drives served team runs.
- [ai_engine.md](ai_engine.md) — Planners and fan-out policy.
- [core_infrastructure.md](core_infrastructure.md) — Identity, security, and runtime configuration primitives.
- [security_config_identity.md](security_config_identity.md) — `Principal` and identity types underlying role authorization.
