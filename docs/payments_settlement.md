...
The design is driven by [ADR-016: Agent Identity and Payment Boundary](docs/architecture/AGENT_IDENTITY_AND_PAYMENT_BOUNDARY.md), layered on the side-effect ledger's exactly-once saga substrate ([ADR-013](docs/architecture/SIDE_EFFECT_LEDGER.md)) and the data-class residency rule ([ADR-012](docs/architecture/DATA_CLASS_RESIDENCY.md)).
...
- Carries a `data_class` ([`ainxt_types::DataClass`](security_config_identity.md#dataclass)) that drives residency: regulated/PII intents are forced in-house.
...
| Module | Relationship |
|---|---|
| [runtime_engine](runtime_engine.md) | `Engine` routes payment-adjacent tool dispatches through the approval gate and payment-boundary resolver; `EngineObo` layers on-behalf-of policy. |
| [server_serving_core](server_serving_core.md) | Surfaces such as `SettlePayment` and `ApprovalCoordinator` turn human or API approvals into the `Approval` inputs the gate evaluates. |
| [core_infrastructure](core_infrastructure.md) | Uses `ainxt_types::DataClass` for residency decisions and relies on `eventlog` / `telemetry` for audit and metrics. |
| [security_config_identity](security_config_identity.md) | `Principal`, `AgentId`, and delegation chains feed into mandate and approval identity. |
...
- It does **not** grant agents the ability to move money; that guarantee lives at the capability-registry / effect-class layer in [admission](admission.md) and [runtime_engine](runtime_engine.md).
...
- [runtime_engine](runtime_engine.md) — engine dispatch, approval gates, and payment-boundary resolver
- [server_serving_core](server_serving_core.md) — served surfaces including `SettlePayment` and `ApprovalCoordinator`
