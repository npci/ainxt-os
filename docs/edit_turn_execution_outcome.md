...
- **`PipelineOutcome`** â€” the exhaustive, serializable result of one edit turn.
- **`CommitApproval`** â€” the unforgeable token that authorizes a renderer to say "done" and allow a commit.
...
- `confidence()` â€” the confidence score that cleared the gate.
- `spot_audit()` â€” whether the commit is flagged for post-commit human spot-audit.
...
- `stage: Stage` â€” which pipeline stage ran.
- `verdict: StageVerdict` â€” `Pass`, `Fail`, `Skipped`, or `Advisory`.
- `deterministic: bool` â€” whether the verdict came from a deterministic tool or model judgment.
...
- `Complete` â†’ `Stage::CommitGate`
- `Capped` â†’ the `blocking_stage`
- `Blocked` â†’ the failing `stage`
...
- `Complete` â†’ commit.
- `Capped` â†’ honest hand-off to a human; no commit.
- `Blocked` â†’ deterministic failure; no commit.
