...
use judge::{JudgeConfig, JudgeOutcome, JudgePipeline};
...
impl KeywordScanResult {
    fn disabled(keyword_scan_safe_score: f32, keyword_scan_block_score: f32) -> Self {
        KeywordScanResult {
            ...
            passed: true,
            score: 0.0,
            keyword_scan_safe_score,
            keyword_scan_block_score,
            reason: String::new(),
        }
    }
            ...
            if resp.status().is_success() {
                ...
                let err_msg = if err_body.is_empty() {
                    ...
                    format!(
                        "HTTP {} — {}",
                        status,
                        err_body.chars().take(120).collect::<String>()
                    )
                };
...
async fn llm_status(State(state): State<Arc<AppState>>) -> Json<LlmStatusResponse> {
    ...
    tracing::info!(
        "[{}] [llm-status] request received — probing all configured models",
        req_id
    );
    ...
    tracing::info!(
        "[{}] [llm-status] probe complete — {} model(s) checked",
        req_id,
        models.len()
    );
    for m in &models {
...
async fn scan(
    ...
    let (scanned_text, _compliance_redaction_count) = if state.layer_compliance {
        state
            .policy_engine
            .check_compliance_ingress(&normalized, provenance)
    } else {
        (normalized.clone(), 0)
    };
    ...
    let text_for_judge = if state.layer_compliance {
        ...
        state
            .policy_engine
            .redact_for_judge(&scanned_text, provenance)
    };
            ...
            PolicyDecision::Deny { policy_id, reason } => {
                respond!(ScanResponse {
                    ...
                    findings: build_findings(
                        &[],
                        &[AuditEntry::now("L2:guardrails-policy", &policy_id, &reason)]
                    ),
                    fenced: req
                        .chunks
                        .iter()
                        .map(|c| guard.fence(c, provenance))
                        .collect(),
                    ...
                    layers: LayerStatus {
                        ...
                        keyword_scan: if state.layer_keyword_scan {
                            KeywordScanResult::not_called(
                                state.keyword_scan_safe_score,
                                state.keyword_scan_block_score,
                            )
                        } else {
                            KeywordScanResult::disabled(
                                state.keyword_scan_safe_score,
                                state.keyword_scan_block_score,
                            )
                        },
                        llm_judges: if state.layer_llm_judges {
                            JudgeResult::not_called(state.judge_confidence)
                        } else {
                            JudgeResult::disabled(state.judge_confidence)
                        },
                    },
    ...
    if state.layer_keyword_scan && heuristic_result {
        if heuristic_score < state.keyword_scan_safe_score {
            ...
            respond!(ScanResponse {
                ...
                layers: LayerStatus {
                    compliance: compliance_result.clone(),
                    guardrails_policy: if state.layer_guardrails_policy {
                        PolicyLayerResult::passed(2)
                    } else {
                        PolicyLayerResult::disabled(2)
                    },
                    keyword_scan: KeywordScanResult::blocked(
                        heuristic_score,
                        state.keyword_scan_safe_score,
                        state.keyword_scan_block_score,
                        format!(
                            "score {heuristic_score:.2} exceeds keyword_scan_block_score {}",
                            state.keyword_scan_block_score
                        )
                    ),
                    llm_judges: if state.layer_llm_judges {
                ...
                audit: vec![AuditEntry::now(
                    ...
                    "",
                    format!(
                        "score {heuristic_score:.2} > keyword_scan_block_score {}",
                        state.keyword_scan_block_score
                    )
                )],
            });
    ...
    let keyword_scan_result = if state.layer_keyword_scan {
        if heuristic_score < state.keyword_scan_safe_score {
            KeywordScanResult::fast_allow(
                heuristic_score,
                state.keyword_scan_safe_score,
                state.keyword_scan_block_score,
            )
        } else {
            KeywordScanResult::escalate(
                heuristic_score,
                state.keyword_scan_safe_score,
                state.keyword_scan_block_score,
            )
    ...
    if state.layer_llm_judges {
        ...
        if let Some(ref pipeline) = *pipeline_guard {
            ...
            match pipeline
                .evaluate(&text_for_judge, provenance, score_to_pass, &req_id)
                .await
            {
                JudgeOutcome::Block {
                    reason,
                    ...
                    respond!(ScanResponse {
                        ...
                        friendly_message: if friendly_message.is_empty() {
                            None
                        } else {
                            Some(friendly_message)
                        },
                        duration_ms: 0,
                        layers: LayerStatus {
                            compliance: compliance_result.clone(),
                            ...
                            llm_judges: JudgeResult::blocked(
                                stage,
                                score,
                                state.judge_confidence,
                                &reason
                            ),
                        },
                        audit: audit_l23,
                ...
                JudgeOutcome::Unavailable { reason } => {
                    ...
                    match state.llm_unavailable.as_str() {
                                    ...
                                    PolicyDecision::Deny {
                                        ...
                                        respond!(ScanResponse {
                                            ...
                                            blocked_by: Some(format!(
                                                "L1:compliance-egress:{policy_id}"
                                            )),
                                            friendly_message: None,
                                            duration_ms: 0,
                                            layers: LayerStatus {
                                                ...
                                                keyword_scan: keyword_scan_result,
                                                llm_judges: JudgeResult::unavailable(
                                                    state.judge_confidence,
                                                    &reason
                                                ),
                                            },
                        ...
                        _ => {
                            // Default: block
                            let audit_unav_block =
                                vec![AuditEntry::now("L4/L5:llm-judge", "", &reason)];
                            respond!(ScanResponse {
                                timestamp: ts.clone(),
                                mode: mode_str.clone(),
                                allowed: false,
                ...
                JudgeOutcome::Allow { stage, score } | JudgeOutcome::Skipped { stage, score } => {
                    ...
                    let fenced_out = if state.layer_compliance {
                        fenced
                            ...
                            .map(|c| {
                                let (redacted, _) =
                                    state.policy_engine.redact_egress(c, provenance);
                                redacted
                            })
                            .collect::<Vec<_>>()
                    } else {
                    ...
                    respond!(ScanResponse {
                        ...
                        layers: LayerStatus {
                            ...
                            keyword_scan: keyword_scan_result,
                            llm_judges: JudgeResult::allowed(
                                judge_stage,
                                judge_score,
                                state.judge_confidence
                            ),
    ...
    let all_disabled = !state.layer_guardrails_policy
        && !state.layer_keyword_scan
        && !state.layer_llm_judges
        && !state.layer_compliance;

    if all_disabled {
        match state.all_layers_disabled.as_str() {
            "block" => {
                let audit_all_disabled = vec![AuditEntry::now(
                    "all-layers-disabled",
                    "",
                    "all defence layers disabled — blocked by policy",
                )];
                respond!(ScanResponse {
                    timestamp: ts,
                    mode: mode_str,
                    ...
                    layers: LayerStatus {
                        ...
                        keyword_scan: KeywordScanResult::disabled(
                            state.keyword_scan_safe_score,
                            state.keyword_scan_block_score
                        ),
...
async fn main() {
    ...
    match &cfg.log_dir {
        Some(dir) => {
            ...
            tracing_subscriber::registry()
                ...
                .init();
            eprintln!(
                "logging: writing hourly rotating logs to {}/ainxt-injection-svc-YYYY-MM-DD-HH.log",
                dir
            );
        }
        None => {
    ...
    let injection_cfg = InjectionDefenseConfig {
        mode: match cfg.mode.trim().to_ascii_lowercase().as_str() {
    ...
    row(format!(
        "  L1  Compliance        [{}]  ainxt-compliance",
        on_off(cfg.layer_compliance)
    ));
    row(format!(
        "  L2  Guardrails+Policy [{}]  ainxt-guardrails",
        on_off(cfg.layer_guardrails_policy)
    ));
    row(format!(
        "  L3  Keyword Scan      [{}]  ainxt-injection",
        on_off(cfg.layer_keyword_scan)
    ));
    row(format!(
        "  L4/5 LLM Judges       [{}]  {}",
        on_off(cfg.layer_llm_judges),
        judges_url
    ));
    row(format!("       Judge 1  : {}", cfg.judge1_model));
    row(format!("       Judge 2  : {}", cfg.judge2_model));
    if cfg.fallback_models.is_empty() {
        row("       Fallback : (none)".to_string());
    } else {
        row(format!(
            "       Fallback : [{}]",
            cfg.fallback_models.join("  →  ")
        ));
        row("                  (tried in order if primary times out)".to_string());
    }
    row(format!(
        "       Max attempts per judge : {}",
        cfg.max_fallback_attempts
    ));
    sep();
    row("  CONFIG FILES".to_string());
    row(format!(
        "  guardrails-policy-rules : {}",
        cfg.guardrails_policy_rules_path
            .as_deref()
            .unwrap_or("built-in")
    ));
    row(format!(
        "  llm-judge-rules         : {}",
        cfg.llm_judge_rules_path.as_deref().unwrap_or("built-in")
    ));
    bot();
    ...
    if let Some(ref rules_path) = llm_judge_rules_path {
        ...
        tokio::spawn(async move {
            ...
            let mut watcher = match recommended_watcher(tx) {
                ...
                Err(e) => {
                    tracing::warn!(
                        "judge: file watcher failed to start: {e} — hot-reload disabled"
                    );
                    return;
                }
            };
            ...
            tracing::info!(
                "judge: watching {} for changes (hot-reload enabled)",
                watch_path
            );
            ...
            loop {
                match rx.recv_timeout(Duration::from_millis(500)) {
                    Ok(Ok(event)) => {
                        ...
                        // Reload
                        tracing::info!(
                            "judge: detected change in {} — reloading rules",
                            watch_path
                        );
                        let new_pipeline =
                            JudgeConfig::from_config(&state_watcher.judge_cfg_snapshot)
