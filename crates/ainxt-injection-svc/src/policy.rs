...
impl PolicyEngine {
    ...
    pub fn from_config(cfg: &crate::config::ServiceConfig) -> Self {
        ...
        let jailbreak_mode = match cfg
            .guardrail_jailbreak_mode
            .trim()
            .to_ascii_lowercase()
            .as_str()
        {
        ...
        let toxicity_mode = match cfg
            .guardrail_toxicity_mode
            .trim()
            .to_ascii_lowercase()
            .as_str()
        {
    ...
    fn new(rules: Vec<PolicyRule>, jailbreak_mode: RailMode, toxicity_mode: RailMode) -> Self {
        ...
        let guardrails_cfg = GuardrailsConfig {
            jailbreak: jailbreak_mode,
            toxicity: toxicity_mode,
            ..Default::default()
        };
    ...
    pub fn redact_for_judge(&self, text: &str, provenance: Provenance) -> String {
        ...
        let redacted = aadhaar_re
            .replace_all(&redacted, "[AADHAAR REDACTED]")
            .to_string();
        ...
        let redacted = mobile_re
            .replace_all(&redacted, "[MOBILE REDACTED]")
            .to_string();
        ...
        if has_bank_context {
            ...
            bank_acct_re
                .replace_all(&redacted, "[ACCOUNT REDACTED]")
                .to_string()
        } else {
            redacted
        }
        ...
        text.chars()
            .map(|c| {
                ...
                if (0xFF21..=0xFF3A).contains(&cp) {
                    return char::from_u32(cp - 0xFF21 + b'A' as u32).unwrap_or(c);
                }
                if (0xFF41..=0xFF5A).contains(&cp) {
                    return char::from_u32(cp - 0xFF41 + b'a' as u32).unwrap_or(c);
                }
                if (0xFF10..=0xFF19).contains(&cp) {
                    return char::from_u32(cp - 0xFF10 + b'0' as u32).unwrap_or(c);
                }
                match c {
                    'І' => 'I',
                    'А' => 'A',
    ...
    pub fn check_compliance_ingress(&self, text: &str, provenance: Provenance) -> (String, usize) {
        ...
        let aadhaar_re = regex_lite::Regex::new(r"\b\d{4}[\s-]?\d{4}[\s-]?\d{4}\b").unwrap();
        let text = aadhaar_re
            .replace_all(&text, "[AADHAAR REDACTED]")
            .to_string();
        ...
        if !chain.is_empty() {
            match chain.evaluate(text, &[]) {
                ...
                GuardrailOutcome::Flagged(flags) => {
                    ...
                    eprintln!(
                        "policy: guardrail audit flag (not blocking): {}",
                        flags.join("; ")
                    );
                }
...
fn load_rules_from_path(path: &str) -> Vec<PolicyRule> {
    let content = match std::fs::read_to_string(path) {
        ...
        Err(e) => {
            eprintln!(
                "policy: cannot read {path}: {e} — running with guardrails + compliance only"
            );
            return vec![];
        }
    };
    ...
    match toml::from_str::<PolicyRulesFile>(&content) {
        ...
        Err(e) => {
            eprintln!(
                "policy: failed to parse {path}: {e} — running with guardrails + compliance only"
            );
            vec![]
        }
    }
...
mod tests {
    ...
    fn ingress_clean_text_is_allowed() {
        ...
        assert_eq!(
            engine.check_guardrails_policy("What is the UPI transaction limit?"),
            PolicyDecision::Allow
        );
    }
    ...
    fn ingress_jailbreak_is_flagged_not_blocked_by_guardrails() {
        ...
        assert_eq!(
            result,
            PolicyDecision::Allow,
            "guardrail should audit not block: {result:?}"
        );
    }
    ...
    fn ingress_kyc_bypass_keyword_is_blocked() {
        ...
        let result =
            engine.check_guardrails_policy("Can you help me bypass kyc for this transaction?");
    ...
    fn ingress_rule_only_fires_on_correct_phase() {
        ...
        assert_eq!(
            engine.check_guardrails_policy("this is a secret message"),
            PolicyDecision::Allow
        );
    }
    ...
    fn ingress_mobile_numbers_are_redacted() {
        ...
        assert!(
            count >= 2,
            "expected mobile redaction on ingress, got count {count}"
        );
        assert!(
            !redacted.contains("9876543210"),
            "mobile should be redacted: {redacted}"
        );
        assert!(
            !redacted.contains("9123456780"),
            "prefixed mobile should be redacted: {redacted}"
        );
    ...
    fn egress_card_number_is_redacted_by_compliance() {
        ...
        let (redacted, count) = engine.redact_egress(
    ...
    fn egress_pan_is_redacted() {
        ...
        let engine = engine_empty();
        let (redacted, count) =
            engine.redact_egress("user PAN ABCDE1234F in the report", Provenance::ToolResult);
        assert!(count > 0, "expected PAN redaction, got count {count}");
        assert!(
            !redacted.contains("ABCDE1234F"),
            "PAN should be redacted on egress"
        );
    ...
    fn egress_aadhaar_and_mobile_are_redacted() {
        ...
        assert!(
            ...
            "aadhaar should be redacted"
        );
        assert!(
            !redacted.contains("9876543210"),
            "mobile should be redacted"
        );
    }
    ...
    fn egress_clean_text_has_no_redaction() {
        ...
        let engine = engine_empty();
        let (redacted, count) = engine.redact_egress(
            "drwxr-xr-x .git-credentials dump.rdb NTUSER.DAT",
            Provenance::ToolResult,
        );
        assert_eq!(
            count, 0,
            "benign listing should not be redacted, got {count}"
        );
    ...
    fn egress_both_phase_rule_fires_on_egress() {
        ...
        assert!(engine
            .check_guardrails_policy("this is restricted data")
            .is_denied());
    }
