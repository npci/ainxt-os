...
struct JudgePolicyFile {
    ...
    #[serde(default)]
    assess_tool_result: AssessSection,
    #[serde(default)]
    cross_check: CrossCheckSection,
    #[serde(default)]
...
struct FallbackSection {
    #[serde(default)]
    unsafe_signals: Vec<String>,
    #[serde(default)]
    safe_signals: Vec<String>,
    #[serde(default)]
    fallback_confidence: Option<f32>,
    #[serde(default)]
    fallback_ambiguous_confidence: Option<f32>,
}
...
impl JudgeConfig {
    ...
    pub fn from_config(cfg: &crate::config::ServiceConfig) -> Option<Self> {
        ...
        let primaries: Vec<&str> = [cfg.judge1_model.as_str(), cfg.judge2_model.as_str()]
            .into_iter()
            .collect();
        ...
        if fallback_models.is_empty() {
            eprintln!(
                "judge: fallback pool is empty — no fallback available if a primary model fails"
            );
        } else {
...
impl LlmJudge {
        ...
        if !rotated.is_empty() {
            eprintln!(
                ...
                models
                    ...
                    .collect::<Vec<_>>()
                    .join(", "),
                if fallback_pool.len() > 1 {
                    judge_index % fallback_pool.len()
                } else {
                    0
                }
            );
    ...
    async fn call(
        ...
        self.call_with_tokens(system, user, judge_id, req_id, self.max_tokens, skip_set)
            .await
    }

    async fn call_with_tokens(
        &self,
        system: &str,
        user: &str,
        judge_id: &'static str,
        ...
        let url = format!(
            "{}/chat/completions",
            self.litellm_url.trim_end_matches('/')
        );
        ...
        for slot in &self.models {
            ...
            if skip_set.lock().unwrap().contains(&slot.model) {
                tracing::info!(
                    "[{}] [{}] model={} result=SKIP reason=request_skip_set",
                    req_id,
                    judge_id,
                    slot.model
                );
                errors.push(format!(
                    "{}: skipped (timed out earlier this request)",
                    slot.model
                ));
                continue;
            }
            ...
            if !slot.circuit.should_try() {
                tracing::info!(
                    "[{}] [{}] model={} result=SKIP reason=circuit_open",
                    req_id,
                    judge_id,
                    slot.model
                );
            ...
            if is_halfopen {
                ...
                let probe_ok = slot
                    .circuit
                    .probe(&probe_url, &self.api_key, &slot.model, &self.client, 10_000)
                    .await;
                if !probe_ok {
                    tracing::info!(
                        "[{}] [{}] model={} result=SKIP reason=probe_failed",
                        req_id,
                        judge_id,
                        slot.model
                    );
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    tracing::warn!(
                        "[{}] [{}] model={} result=FAILED duration={}ms reason=\"{}\"",
                        req_id,
                        judge_id,
                        slot.model,
                ...
                if status.as_u16() == 500 || status.as_u16() == 503 {
                    for backoff_ms in [1_000u64, 2_000, 4_000] {
                        ...
                        if retry_status.is_success() {
                            ...
                            if !content.is_empty() {
                                slot.circuit.record_success();
                                let (verdict, confidence, reason, category, friendly_message) =
                                    match parse_verdict(
                                        &content,
                                        &slot.model,
                        ...
                        tracing::warn!(
                            "[{}] [{}] model={} retry still failing status={}",
                            req_id,
                            judge_id,
                            slot.model,
                            retry_status
    ...
    pub async fn assess(
        ...
        let (system_prompt, user_template) = if provenance.is_trusted() {
            (
                &self.assess_system_prompt,
                &self.assess_user_prompt_template,
            )
        } else {
            (
                &self.assess_tool_result_system_prompt,
                &self.assess_tool_result_user_prompt_template,
            )
        };
        ...
        self.call(system_prompt, &user_prompt, judge_id, req_id, skip_set)
            .await
    }
...
impl JudgePipeline {
    ...
    pub async fn evaluate(
        ...
        let (r1, r2) = tokio::join!(
            self.judge1
                .assess(text, provenance, "judge1", req_id, &skip_set),
            self.judge2
                .assess(text, provenance, "judge2", req_id, &skip_set),
        );
        ...
        if v1.is_none() {
            tracing::warn!(
                "[{}] judge1=UNAVAILABLE — proceeding with judge2 only",
                req_id
            );
        ...
        if v2.is_none() {
            tracing::warn!(
                "[{}] judge2=UNAVAILABLE — proceeding with judge1 only",
                req_id
            );
            return JudgeOutcome::Unavailable {
                reason: errors[0].clone(),
                    ...
                    && j1.verdict == j2.verdict
                {
                    ...
                    let outcome = match j1.verdict {
                        Verdict::Unsafe => {
                            ...
                            let s1_unsafe = [&j1.verdict, &j2.verdict]
                                .iter()
                                .filter(|v| ***v == Verdict::Unsafe)
                                .count();
                            let s1_safe = [&j1.verdict, &j2.verdict]
                                .iter()
                            ...
                            JudgeOutcome::Block {
                                ...
                                score: avg_score,
                                friendly_message: if j1.friendly_message.is_empty() {
                                    j2.friendly_message.clone()
                                } else {
                                    j1.friendly_message.clone()
                                },
                            }
                        }
                        Verdict::Safe => {
        ...
        tracing::info!(
            "[{}] stage=stage2_cross_validation (judges disagreed or below threshold)",
            req_id
        );
        ...
        if confident.is_empty() {
            let all_unsafe = all_verdicts
                .iter()
                .filter(|v| v.verdict == Verdict::Unsafe)
                .count();
            let all_safe = all_verdicts
                .iter()
                .filter(|v| v.verdict == Verdict::Safe)
                .count();
            tracing::info!(
        ...
        let avg_score =
            confident.iter().map(|v| v.confidence).sum::<f32>() / confident.len() as f32;
        let unsafe_count = confident
            .iter()
            .filter(|v| v.verdict == Verdict::Unsafe)
            .count();
        let safe_count = confident
            .iter()
            .filter(|v| v.verdict == Verdict::Safe)
            .count();
        ...
        if unsafe_count > safe_count {
            ...
            let friendly_message = confident
                .iter()
                .filter(|v| v.verdict == Verdict::Unsafe)
            ...
            match self.cfg.tie_behaviour {
                TieBehaviour::Block => {
                    let reason =
                        format!("tie ({unsafe_count} unsafe vs {safe_count} safe) — fail-closed");
                    tracing::info!(
...
fn extract_content(raw_body: &str) -> String {
    if let Ok(llm_resp) = serde_json::from_str::<LiteLlmResponse>(raw_body) {
        if let Some(c) = llm_resp.choices.first() {
            ...
            let reasoning = c.message.reasoning_content.as_ref().or_else(|| {
                c.message
...
fn fallback_verdict(
    ...
    if unsafe_hit && !safe_hit {
        Ok((
            Verdict::Unsafe,
            fallback_confidence,
            "[plain-text fallback] suspicious prose from judge".into(),
            "other".into(),
            String::new(),
        ))
        ...
        Ok((
            Verdict::Safe,
            fallback_ambiguous_confidence,
            "[plain-text fallback] ambiguous prose — no clear verdict".into(),
            "other".into(),
            String::new(),
        ))
    }
...
mod tests {
    ...
    fn builtin_toml_has_tool_result_prompt() {
        ...
        assert!(
            !policy.assess_tool_result_system_prompt.is_empty(),
        ...
        assert_ne!(
            policy.assess_tool_result_system_prompt, policy.assess_system_prompt,
        ...
        assert!(
            p.contains("tool result") || p.contains("tool-result"),
            "tool-result prompt must state it is judging tool results"
        );
        assert!(
            p.contains("redaction"),
...
"#;
        ...
        assert_eq!(
            policy.assess_tool_result_system_prompt, "INGRESS PROMPT",
