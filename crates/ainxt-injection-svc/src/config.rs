...
impl ServiceConfig {
    ...
    pub fn load() -> Self {
        ...
        let candidates: Vec<String> = if let Some(p) = toml_path {
            ...
            if let Some(workspace_root) = std::env::current_exe()
                .ok()
                .and_then(|p| p.parent().map(|p| p.to_path_buf()))
                .and_then(|p| p.parent().map(|p| p.to_path_buf()))
                .and_then(|p| p.parent().map(|p| p.to_path_buf()))
        ...
        for candidate in &candidates {
            if let Ok(content) = std::fs::read_to_string(candidate) {
                match toml::from_str::<ConfigFile>(&content) {
                    ...
                    Err(e) => {
                        eprintln!(
                            "config: failed to parse {candidate}: {e} — trying next location"
                        );
                    }
                }
            }
    ...
    fn apply_toml(&mut self, file: &ConfigFile) {
        // Server
        if let Some(ref v) = file.server.host {
            self.host = v.clone();
        }
        if let Some(v) = file.server.port {
            self.port = v;
        }
        if let Some(ref v) = file.server.mode {
            self.mode = v.clone();
        }

        // Judges
        if let Some(ref v) = file.judges.litellm_url {
            self.litellm_url = Some(v.clone());
        }
        if let Some(ref v) = file.judges.litellm_api_key {
            self.litellm_api_key = Some(v.clone());
        }
        if let Some(ref v) = file.judges.judge1_model {
            self.judge1_model = v.clone();
        }
        if let Some(ref v) = file.judges.judge2_model {
            self.judge2_model = v.clone();
        }
        if let Some(v) = file.judges.confidence_threshold {
            self.confidence_threshold = v;
        }

        if let Some(v) = file.judges.skip_cross_on_consensus {
            self.skip_cross_on_consensus = v;
        }
        if let Some(ref v) = file.judges.tie_behaviour {
            self.tie_behaviour = v.clone();
        }
        if let Some(v) = file.judges.timeout_ms {
            self.timeout_ms = v;
        }
        if let Some(ref v) = file.judges.llm_unavailable {
            self.llm_unavailable = v.clone();
        }
        if let Some(ref v) = file.judges.fallback_models {
            self.fallback_models = v.clone();
        }
        if let Some(v) = file.judges.max_fallback_attempts {
            self.max_fallback_attempts = v;
        }
        if let Some(v) = file.judges.circuit_breaker_failures {
            self.circuit_breaker_failures = v;
        }
        if let Some(v) = file.judges.circuit_breaker_timeout_s {
            self.circuit_breaker_timeout_s = v;
        }

        // Server-level safety
        if let Some(ref v) = file.server.all_layers_disabled {
            self.all_layers_disabled = v.clone();
        }

        // Policy
        // [config] section — all external file paths in one place
        if let Some(ref v) = file.config.llm_judge_rules_path {
            self.llm_judge_rules_path = Some(v.clone());
        }

        if let Some(ref v) = file.config.guardrails_policy_rules_path {
            self.guardrails_policy_rules_path = Some(v.clone());
        }
        if let Some(ref v) = file.guardrails.rules_path {
            self.guardrails_policy_rules_path = Some(v.clone());
        }
        if let Some(ref v) = file.guardrails.guardrail_jailbreak_mode {
            self.guardrail_jailbreak_mode = v.clone();
        }
        if let Some(ref v) = file.guardrails.guardrail_toxicity_mode {
            self.guardrail_toxicity_mode = v.clone();
        }

        // Server extras
        if let Some(v) = file.server.max_chunks {
            self.max_chunks = v;
        }
        if let Some(ref v) = file.server.log_dir {
            self.log_dir = Some(v.clone());
        }
        if let Some(ref v) = file.server.log_level {
            self.log_level = v.clone();
        }

        // Judge tuning
        if let Some(ref v) = file.judges.policy_path {
            self.llm_judge_rules_path = Some(v.clone());
        }
        if let Some(v) = file.judges.temperature {
            self.judge_temperature = v;
        }
        if let Some(v) = file.judges.max_tokens {
            self.judge_max_tokens = v;
        }
        if let Some(v) = file.judges.cross_check_max_tokens {
            self.judge_cross_check_max_tokens = v;
        } else if let Some(v) = file.judges.max_tokens {
            self.judge_cross_check_max_tokens = v;
        }
        if !file.judges.model_max_tokens.is_empty() {
            self.judge_model_max_tokens = file.judges.model_max_tokens.clone();
        }
        if let Some(v) = file.judges.accept_invalid_certs {
            self.judge_accept_invalid_certs = v;
        }
        if let Some(v) = file.judges.pool_max_idle_per_host {
            self.judge_pool_max_idle_per_host = v;
        }
        if let Some(v) = file.judges.pool_idle_timeout_secs {
            self.judge_pool_idle_timeout_secs = v;
        }

        // Layer toggles
        if let Some(v) = file.layers.compliance_layer {
            self.layer_compliance = v;
        }
        if let Some(v) = file.layers.guardrails_policy_layer {
            self.layer_guardrails_policy = v;
        }
        if let Some(v) = file.layers.keyword_scan_layer {
            self.layer_keyword_scan = v;
        }
        if let Some(v) = file.layers.llm_judges_layer {
            self.layer_llm_judges = v;
        }

        // [keyword_scan] section
        if let Some(v) = file.keyword_scan.safe_score {
            self.keyword_scan_safe_score = v;
        }
        if let Some(v) = file.keyword_scan.block_score {
            self.keyword_scan_block_score = v;
        }
    }
    ...
    fn apply_env(&mut self) {
        // Server
        if let Ok(v) = std::env::var("AINXT_INJECTION_SVC_HOST") {
            self.host = v;
        }
        ...
        if let Ok(v) = std::env::var("AINXT_INJECTION_MODE") {
            self.mode = v;
        }

        // Judges
        if let Ok(v) = std::env::var("JUDGE_LITELLM_URL") {
            self.litellm_url = Some(v);
        }
        if let Ok(v) = std::env::var("JUDGE_LITELLM_API_KEY") {
            self.litellm_api_key = Some(v);
        }
        if let Ok(v) = std::env::var("JUDGE1_MODEL") {
            self.judge1_model = v;
        }
        if let Ok(v) = std::env::var("JUDGE2_MODEL") {
            self.judge2_model = v;
        }
        ...
        if let Ok(v) = std::env::var("KEYWORD_SCAN_SAFE_SCORE") {
            if let Ok(f) = v.parse() {
                self.keyword_scan_safe_score = f;
            }
        }
        if let Ok(v) = std::env::var("KEYWORD_SCAN_BLOCK_SCORE") {
            if let Ok(f) = v.parse() {
                self.keyword_scan_block_score = f;
            }
        }
        if let Ok(v) = std::env::var("JUDGE_SKIP_CROSS_ON_CONSENSUS") {
            self.skip_cross_on_consensus = !v.trim().eq_ignore_ascii_case("false");
        }
        if let Ok(v) = std::env::var("JUDGE_TIE_BEHAVIOUR") {
            self.tie_behaviour = v;
        }
        ...
        if let Ok(v) = std::env::var("JUDGE_CB_TIMEOUT_S") {
            if let Ok(n) = v.parse() {
                self.circuit_breaker_timeout_s = n;
            }
        }
        ...
        // Policy
        if let Ok(v) = std::env::var("GUARDRAILS_POLICY_RULES_PATH") {
            self.guardrails_policy_rules_path = Some(v);
        }
        if let Ok(v) = std::env::var("LLM_JUDGE_RULES_PATH") {
            self.llm_judge_rules_path = Some(v);
        }

        if let Ok(v) = std::env::var("JUDGE_TEMPERATURE") {
            if let Ok(f) = v.parse() {
                self.judge_temperature = f;
            }
        }
        if let Ok(v) = std::env::var("JUDGE_MAX_TOKENS") {
            if let Ok(n) = v.parse() {
                self.judge_max_tokens = n;
            }
        }
        if let Ok(v) = std::env::var("JUDGE_CROSS_CHECK_MAX_TOKENS") {
            if let Ok(n) = v.parse() {
                self.judge_cross_check_max_tokens = n;
            }
        }
        if let Ok(v) = std::env::var("JUDGE_ACCEPT_INVALID_CERTS") {
            self.judge_accept_invalid_certs = !v.trim().eq_ignore_ascii_case("false");
        }
        if let Ok(v) = std::env::var("JUDGE_POOL_MAX_IDLE_PER_HOST") {
            if let Ok(n) = v.parse() {
                self.judge_pool_max_idle_per_host = n;
            }
        }
        if let Ok(v) = std::env::var("JUDGE_POOL_IDLE_TIMEOUT_SECS") {
            if let Ok(n) = v.parse() {
                self.judge_pool_idle_timeout_secs = n;
            }
        }
        if let Ok(v) = std::env::var("SERVER_MAX_CHUNKS") {
            if let Ok(n) = v.parse() {
                self.max_chunks = n;
            }
        }
        ...
        if let Ok(v) = std::env::var("GUARDRAIL_JAILBREAK_MODE") {
            self.guardrail_jailbreak_mode = v;
        }
        if let Ok(v) = std::env::var("GUARDRAIL_TOXICITY_MODE") {
            self.guardrail_toxicity_mode = v;
        }

        // Layer toggles
        if let Ok(v) = std::env::var("COMPLIANCE_LAYER") {
            self.layer_compliance = !v.trim().eq_ignore_ascii_case("false");
        }
        if let Ok(v) = std::env::var("GUARDRAILS_POLICY_LAYER") {
            self.layer_guardrails_policy = !v.trim().eq_ignore_ascii_case("false");
        }
        if let Ok(v) = std::env::var("KEYWORD_SCAN_LAYER") {
            self.layer_keyword_scan = !v.trim().eq_ignore_ascii_case("false");
        }
        if let Ok(v) = std::env::var("LLM_JUDGES_LAYER") {
            self.layer_llm_judges = !v.trim().eq_ignore_ascii_case("false");
        }
    }
}
    ...
    fn toml_overlay_applies_values() {
        ...
        let file = ConfigFile {
            config: ConfigFilesSection {
                ...
                guardrails_policy_rules_path: Some(
                    "/etc/ainxt/guardrails-policy-rules.toml".into(),
                ),
            },
            server: ServerToml {
        ...
        assert_eq!(
            cfg.fallback_models,
            vec!["model-fallback-a", "model-fallback-b"]
        );
        assert_eq!(cfg.max_fallback_attempts, 3);
        assert_eq!(cfg.circuit_breaker_failures, 5);
        assert_eq!(cfg.circuit_breaker_timeout_s, 60);
        assert_eq!(
            cfg.guardrails_policy_rules_path.as_deref(),
            Some("/etc/ainxt/guardrails-policy-rules.toml")
        );
