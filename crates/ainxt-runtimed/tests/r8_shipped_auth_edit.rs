...
async fn r8_jwt_sso_authenticator_selectable_and_verified() {
    let secret = "r8-test-secret";
    let loaded = loaded(&format!(
        "authenticator = \"jwt-sso\"\njwt_hs256_secret = \"{secret}\"\n"
    ));
    let assembled = assemble_surface(&loaded, "chat").expect("assemble chat surface");
    let full = assemble_full(&loaded, assembled).expect("jwt-sso with a secret assembles");
    assert!(
        full.report
            .iter()
            .any(|l| l.contains("authenticator: jwt-sso")),
        "the report must name the SELECTED authenticator: {:?}",
        full.report
    );

    let addr = serve(&loaded).await;
    let client = reqwest::Client::new();
    let graph_body = serde_json::json!({"op":"traverse","start":"root","max_depth":1}).to_string();

    // Header-spoofing no longer widens identity: a forged X-AInxt-User with no Bearer is refused.
    let spoofed = client
        .post(format!("http://{addr}/graph"))
        .header(reqwest::header::CONTENT_TYPE, "application/json")
        .header("x-ainxt-user", "alice")
        .body(graph_body.clone())
        .send()
        .await
    ...
    assert_eq!(
        ...
        401,
        "jwt-sso must reject the forwarded identity header once selected"
    );
    ...
    let garbage = client
        ...
        .send()
        .await
        .expect("garbage bearer request");
    assert_eq!(
        garbage.status().as_u16(),
        401,
        "a malformed bearer token must be rejected"
    );

    // A validly-signed HS256 token is accepted — identity comes from the verified claims.
    let token = mint_hs256(
        secret.as_bytes(),
        serde_json::json!({"sub": "alice", "role": "admin"}),
    );
    let valid = client
        .post(format!("http://{addr}/graph"))
        .header(reqwest::header::CONTENT_TYPE, "application/json")
    ...
    assert_ne!(
        valid.status().as_u16(),
        401,
        "a validly-signed HS256 token must be accepted"
    );
}

#[tokio::test(flavor = "multi_thread")]
async fn r8_jwt_sso_without_a_secret_fails_closed_at_assembly() {
    let loaded = loaded("authenticator = \"jwt-sso\"\n");
    let assembled = assemble_surface(&loaded, "chat").expect("assemble chat surface");
    match assemble_full(&loaded, assembled) {
        Err(AssembleError::Config(msg)) => assert!(
            msg.contains("jwt_hs256_secret"),
            "the refusal must name the missing key: {msg}"
        ),
        Err(other) => panic!("expected AssembleError::Config, got {other:?}"),
        Ok(_) => panic!("jwt-sso without a secret must fail closed, never silently downgrade"),
    }
}
