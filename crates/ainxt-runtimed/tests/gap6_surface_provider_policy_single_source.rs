...
/// The CLASSIFIER half of the fix (the genuinely live bypass): `sdlc`'s CANONICAL model policy already
/// sets `allowed_providers = ["claude", "gpt"]` (no override needed). Configure a disallowed `gemini`
/// provider (kind = `local`, so no API key is needed to make it classifier-eligible — isolating the
/// policy check from key presence) ahead of an allowed `gpt` (also `local`, same reason). Before the
/// fix, `build_chat_classifier_model` scanned `loaded.runtime.models` UNFILTERED and picked the FIRST
/// OpenAI-schema-or-local candidate in file order — `gemini`, a provider `sdlc`'s policy explicitly
/// excludes for its confidential-cleared repo turns. After the fix, the classifier only ever sees the
/// SAME surface-narrowed view the router is built from, so `gemini` is invisible to it and `gpt` (the
/// allow-listed provider) is what gets wired.
#[test]
fn r_allowed_providers_narrows_the_stage2_classifier_candidate_set_not_just_the_router() {
    let loaded = load_layered(&[(
        "deployment",
        "version = 1\n\
    ...
    // The NEW assertion: the Stage-2 classifier must have been wired against the allow-listed `gpt`,
    // never the excluded `gemini` — proving the classifier's candidate set is now the SAME
    // surface-narrowed view, not the raw unfiltered config. Pre-fix, `gemini` (listed first) would have
    // won this selection since nothing filtered it out for the classifier call site specifically.
    assert!(
        assembled
            .report
            .iter()
            .any(|r| r.contains("Stage-2 model-backed constrained intent classifier wired")),
        "an allow-listed local provider must still be classifier-eligible after the policy narrows the \
         candidate set: {:?}",
        assembled.report
    );
}
