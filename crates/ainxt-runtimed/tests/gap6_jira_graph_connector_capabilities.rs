...
fn gap6_jira_and_graph_capabilities_registered_on_real_composition_root_and_fail_closed_for_real() {
    ...
    let cases: &[(&str, &str)] = &[
        ("jira.get_issue", r#"{"key":"ABC-123"}"#),
        ("jira.add_comment", r#"{"key":"ABC-123","body":"hi"}"#),
        ("graph.get_me", "{}"),
        ("graph.list_messages", r#"{"top":5}"#),
        (
            "graph.send_mail",
        ...
        match registry.dispatch_for("alice", name, args) {
            ...
            const PIPELINE_ERRORS: [&str; 6] = [
                ...
                assert!(
                    ...
                    "'{name}' must fail CLOSED with an honest connector-pipeline error \
                     (admission/egress/token/transport) — not a fabricated success or an \
