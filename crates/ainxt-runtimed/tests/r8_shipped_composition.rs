...
fn r8_shipped_defaults_turn_guardrails_and_injection_on() {
    // The raw config (no shipped base) still defaults guardrails OFF, so the shipped base is what
    // turns them on. Injection is no longer part of that story: `InjectionConfig::default()` is now
    // `Enforce` (secure-by-default), so it is ON even without the shipped base — assert that
    // directly rather than a stale "defaults OFF" baseline.
    ...
    assert!(
        ...
        "injection is secure-by-default (InjectionConfig::default() == Enforce), shipped base or not"
    );
