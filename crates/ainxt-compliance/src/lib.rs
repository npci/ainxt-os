...
fn is_after_identifier_and_before_extension(bytes: &[u8], start: usize, end: usize) -> bool {
    ...
    let has_identifier_prefix = prev.is_ascii_alphabetic() || prev == b'_' || prev == b'-';
    ...
    for marker in MARKERS {
        ...
        while let Some(rel) = lower[from..].find(marker) {
            ...
            if bytes[k] == b'=' || bytes[k] == b':' {
                ...
                sep_kind = SecretSeparator::NaturalLanguage;
                k += 2; // skip "->"
            } else if k + 2 < n && bytes[k] == 0xE2 && bytes[k + 1] == 0x86 && bytes[k + 2] == 0x92
            {
                // '→' (U+2192)
                sep_kind = SecretSeparator::NaturalLanguage;
                k += 3;
...
fn detect_credential_shaped_values(text: &str, out: &mut Vec<Span>) {
    ...
    while i < n {
        ...
        if is_followed_by_file_extension(bytes, end) || is_preceded_by_path_separator(bytes, start)
        {
            continue;
        }
...
mod tests {
    ...
    fn timestamp_in_filename_is_not_redacted() {
        ...
        assert!(
            out.contains("202604281514"),
            "timestamp in filename got redacted: {out}"
        );
        assert_eq!(count, 0);
    }
    ...
    fn bare_timestamp_filename_is_not_redacted() {
        ...
        assert!(
            out.contains("202604281514"),
            "bare timestamp filename redacted: {out}"
        );
        assert_eq!(count, 0);
    }
    ...
    fn natural_language_password_prose_is_not_redacted() {
        ...
        assert_eq!(
            out, "a strong password is important for security",
            "prose was mangled: {out}"
        );
        assert_eq!(count, 0);
    }
    ...
    fn passphrase_disclosure_is_redacted() {
        ...
        assert!(
            !out.contains("Test@12345"),
            "passphrase value leaked: {out}"
        );
    }
    ...
    fn utf8_secret_disclosure_still_redacts_across_multibyte_prose() {
        ...
        assert!(
            !out.contains("Test@12345"),
            "utf-8 prose broke redaction: {out}"
        );
    }
