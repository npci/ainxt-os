...
fn detect_credential_shaped_values(text: &str, out: &mut Vec<Span>) {
    let bytes = text.as_bytes();
    let n = bytes.len();
    // Token delimiter set excludes `=`/`:` so `password=hunter2` splits into
    // `password` + `hunter2` (letting detect_marked_secrets redact cleanly).
...
mod tests {
    ...
    #[test]
    fn colon_assignment_secret_is_redacted() {
        let (out, _) = red("token: abc123def456ghi");
        assert!(
            !out.contains("abc123def456ghi"),
            "colon secret leaked: {out}"
        );
    }
    ...
    #[test]
    fn natural_language_apikey_disclosure_is_redacted() {
        let (out, _) = red("Their apikey is sk_test_abc123def456");
        assert!(!out.contains("sk_test_abc123def456"), "value leaked: {out}");
        assert!(out.contains("[REDACTED-SECRET]"));
    }
