...
//! no syscall filter. A deployment that needs that (a `--network none`, memory/CPU-capped container
//! tier) needs a live container daemon, which is genuinely infra — out of scope for this offline
//! runtime crate. This executor is the real, immediately-usable tier that does not require any infra
//! to exist or be tested.
//!
...
impl SkillExecutor for NativeProcessSkillExecutor {
    fn execute(&self, skill: &SkillManifest, user_input: &str) -> Result<String, SkillError> {
        ...
        // Read capped stdout on its OWN thread, bounded by a channel receive rather than a raw
        // blocking read on the main thread. `kill()` sends SIGKILL to the immediate child only; if
        // the interpreter forked a grandchild for the final command instead of exec'ing into it (a
        // real, observed cross-platform/shell difference, not guaranteed by POSIX), that orphaned
        // grandchild can keep the stdout pipe's write end open long after the watcher's kill — so
        // this read must never be allowed to block execute() past the watcher's own timeout budget.
        ...
        thread::spawn(move || {
            ...
            // The receiver may already be gone (execute() gave up after the timeout+grace budget);
            // that is not a failure the reader thread needs to do anything about, it simply exits.
            let _ = reader_tx.send((buf, overflow));
        });

        // Never wait longer than the configured timeout plus a fixed grace period for the kill to
        // actually land, regardless of whether the reader thread above ever sees EOF. The grace is
        // deliberately small: it only needs to cover scheduling jitter for the watcher's own
        // kill(), not a pipe that may never close — waiting longer for that would not help.
        let (buf, overflow) = reader_rx
            .recv_timeout(self.timeout + Duration::from_millis(500))
            .unwrap_or_default();
