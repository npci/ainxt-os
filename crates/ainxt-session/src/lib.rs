...
/// Best-effort rendering of a [`catch_unwind`](futures_util::FutureExt::catch_unwind) payload.
/// `panic!("literal")` yields a `&'static str`; a formatted `panic!("{x}")` yields a `String`.
/// Anything else (a non-standard panic payload) has no printable form, so say so rather than
/// inventing one.
...
/// Tell the caller the turn died, then close the stream. Best-effort on purpose: a receiver that
/// has already hung up (client disconnected) is the normal case, not an error to report.
async fn terminate_stream(sink: &mpsc::Sender<Event>, msg: &str) {
    let _ = sink.send(Event::Error(msg.to_string())).await;
...
async fn process(
    ...
    // The sink is MOVED into the handler, so a panicking or timed-out turn DROPS it — which the
    // HTTP layer renders as a completed SSE stream carrying zero events: `200 OK`, correct
    // content-type, nothing in the body. A caller cannot distinguish that from a successful empty
    // answer, cannot retry it and cannot report it. Isolating the failure is right; swallowing it
    // is not. Keep a handle so the supervisor can always terminate the caller's stream itself.
    ...
    let res = match tokio::time::timeout(turn_timeout, turn).await {
        // Completed (ok or turn-error) — the handler owns its own terminal events.
        Ok(Ok(res)) => res,
        Ok(Err(panic)) => {
            // Carry the panic PAYLOAD through: it is the only pointer to the real defect, and it
            // is lost forever once this `Box<dyn Any>` is dropped.
            let msg = format!("turn panicked: {}", panic_message(&*panic));
            terminate_stream(&fallback_sink, &msg).await;
            Err(TurnError::Internal(msg))
        }
        Err(_elapsed) => {
            // Dropping the future frees it.
            let msg = "turn timed out".to_string();
            terminate_stream(&fallback_sink, &msg).await;
            Err(TurnError::Internal(msg))
        }
    };
