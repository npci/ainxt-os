...
fn r12_episodic_to_semantic_promotion_pipeline() {
    ...
    assert!(
        store.get("prom-0").unwrap().is_authoritative(),
        "personal promotion usable at once"
    );
    assert_eq!(
        store.get("prom-4").unwrap().governance,
    ...
    assert_eq!(store.get("e1").unwrap().kind, MemoryKind::Episodic);
...
fn r12_versioned_per_type_schema_registry_enforced_on_write() {
    ...
    assert_eq!(
        store.get("sec-1").unwrap().schema_version,
        3,
    ...
    assert_eq!(
        store.get("lib-1").unwrap().schema_version,
        1,
