...
use ainxt_memory::{
    AccessScope, DurableMemoryStore, GovernanceState, MemoryItem, MemoryKind, MemorySqlBackend,
    MemoryStore, OrgPayload, Principal, Provenance, Scope, CAP_APPROVE,
};
...
fn r3_durable_store_survives_reopen() {
    ...
    // ---- session 1: write governed knowledge + a PII-bearing fact + erase a subject ----
    {
        ...
        assert_eq!(
            store.get_unchecked("oki-1").unwrap().governance,
    ...
    let oki = reopened
        .get_unchecked("oki-1")
        .expect("OKI must survive reopen");
    ...
    assert!(reopened.get_unchecked("fact-bob").is_none());
