...
pub const PROVIDER_KINDS: [&str; 5] = ["offline", "local", "anthropic", "open-ai-schema", "gemini"];
...
pub fn validate(s: &Settings) -> Result<(), String> {
    ...
    if !s.provider_base_url.is_empty()
        && !(s.provider_base_url.starts_with("http://")
            || s.provider_base_url.starts_with("https://"))
    {
...
"#;
    ...
    fn round_trip_through_load_preserves_the_subset() {
        ...
        assert!(
            !back.persist_sessions,
            "persistence off must round-trip as off"
        );
    ...
    fn persistence_toggle_writes_and_removes_the_directory_key() {
        ...
        assert!(doc
            .to_string()
            .contains(r#"chat_sessions_dir = "sessions""#));
        s.persist_sessions = false;
        apply(&mut doc, &s);
    ...
    fn validate_rejects_what_the_daemon_would_refuse() {
        ...
        assert!(
            validate(&s).is_err(),
            "local without a base_url must be rejected"
        );
        s.provider_base_url = "localhost:11434".into();
        assert!(
            validate(&s).is_err(),
            "a scheme-less endpoint must be rejected"
        );
        s.provider_base_url = "http://localhost:11434/v1".into();
        ...
        assert!(
            validate(&s).is_err(),
            "an invalid guardrail mode must be rejected"
        );
    }
    ...
    fn sanitize_drops_empty_values() {
        ...
        assert!(
            result.is_empty(),
            "blank credential must be silently dropped"
        );
    }
    ...
    fn sanitize_accepts_values_with_printable_special_chars() {
        ...
        assert_eq!(
            result.len(),
            1,
            "printable non-control chars in value must be accepted"
        );
    }
