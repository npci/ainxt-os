...
async fn run() -> Result<(), String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--runtimed" => {
                i += 1;
                daemon_override = Some(PathBuf::from(args.get(i).ok_or("--runtimed needs a path")?));
            }
    ...
    let listener = tokio::net::TcpListener::bind(addr)
        .await
        .map_err(|e| format!("cannot bind the console to {addr}: {e}\nTry: ainxt-os --port <other>"))?;
...
/// The approved AiNxt icon, embedded from `AINxt_logo_icon/AiNxt Icon_r.svg` (the brand-approved
...
async fn serve_html() -> impl IntoResponse {
    ([(header::CONTENT_TYPE, "text/html; charset=utf-8")], CONSOLE_HTML)
}
...
async fn api_chat(State(app): State<Shared>, Json(body): Json<ChatBody>) -> Response {
    let (port, token, session) = {
        ...
        let token = jwt::mint(
            app.secret.as_bytes(),
            &app.console.identity,
            now_unix(),
            TOKEN_TTL_SECS,
        );
        (app.settings.port, token, format!("console-{}", std::process::id()))
    };
...
async fn api_post_settings(State(app): State<Shared>, Json(body): Json<SaveBody>) -> Response {
    ...
    let mut doc = match current.parse::<toml_edit::DocumentMut>() {
        Ok(d) => d,
        Err(e) => return bad(format!("The current configuration file could not be parsed: {e}")),
    };
    ...
    // The daemon is the authority on validity.
    let env: Vec<(String, String)> = console_state
        .secrets
        .iter()
        .filter(|(_, v)| !v.is_empty())
        .map(|(k, v)| (k.clone(), v.clone()))
    ...
    Json(SaveResult { ok: true, error: None }).into_response()
}
