...
mod tests {
    ...
    fn limited() -> (Duration, usize) {
        // A "trivial" real cargo invocation is fast in isolation, but this helper backs tests that
        // spawn a nested cargo process while `cargo test --workspace` is itself running dozens of
        // tests in parallel on a 2-vCPU CI runner -- 20s was tight enough to flake under that
        // contention (observed: killed after 20s with no output at all). 60s keeps the same
        // real-compiler assertions, just with headroom for a loaded shared runner.
        (Duration::from_secs(60), DEFAULT_MAX_OUTPUT_BYTES)
    }
