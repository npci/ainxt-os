...
impl std::fmt::Debug for ConnectorCallError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ConnectorCallError::Unavailable(m) => f.debug_tuple("Unavailable").field(m).finish(),
...
impl ConnectorGateway {
    ...
    pub fn begin_authorization(
        ...
        let start =
            begin_and_store(self.pending.as_ref(), provider, scopes, now_unix).map_err(|_| {
                ConnectorCallError::Token("authorization flow could not be started".to_string())
            })?;
    ...
    pub fn complete_callback(
        ...
        let blob = serde_json::to_vec(&token_set)
            .map_err(|_| ConnectorCallError::Token("token serialization failed".to_string()))?;
        ...
        fn execute_as(&self, args: &str, caller: Option<&str>) -> Result<String, ToolError> {
                ...
                .invoke_in(&self.tenant, &principal, now, self.data_class, prepared)
            {
                ...
                Err(e) => Err(ToolError::Execution(
                    e.sanitized_client_message().to_string(),
                )),
            }
        }
    }
