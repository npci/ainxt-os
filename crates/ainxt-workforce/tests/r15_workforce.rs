...
// ================================================================== HIGH
// Publish-time compliance/pre-receive gate must scan the FULL citizen-authored body (charter prose,
// agent personas, capability/connector/knowledge names), not a stripped 5-field identity manifest.

#[test]
fn r15_publish_scans_citizen_authored_body_not_just_identity_fields() {
    // A secret marker hidden in free text a citizen actually types: the charter responsibility. None
    // of the identity fields (id/payment_boundary/owner/codeowners_group/residency) carry it.
    let mut leaky = passing_spec("leaky-charter");
    leaky
        .charter
        .responsibilities
        .push("note: SECRET=hunter2 do not commit".into());
    let pass = Breaker::gate(&leaky.clone().validate().unwrap(), &CompliantExecutor)
        .expect("spec + adversarial run are otherwise clean");
    let validated = leaky.validate().unwrap();
    match breaker::publish(validated, &pass, &gov_for("leaky-charter", "support-leads")) {
        Err(PublishError::CiGate(_)) => {}
        other => panic!(
            "pre-receive gate must catch a secret marker embedded in the AUTHORED charter body, got {other:?}"
        ),
    }

    // Same for a marker hidden in an agent's persona free text.
    let mut leaky_persona = passing_spec("leaky-persona");
    leaky_persona.agents[0].persona = "an L1 support persona. API_KEY=abcdef1234567890".into();
    let pass = Breaker::gate(
        &leaky_persona.clone().validate().unwrap(),
        &CompliantExecutor,
    )
    .unwrap();
    let validated = leaky_persona.validate().unwrap();
    match breaker::publish(validated, &pass, &gov_for("leaky-persona", "support-leads")) {
        Err(PublishError::CiGate(_)) => {}
        other => panic!("pre-receive gate must catch a secret marker embedded in an agent persona, got {other:?}"),
    }

    // Control: the same role with NO embedded marker publishes clean — the fix is discriminating, not
    // a blanket rejection.
    let clean = passing_spec("clean-charter");
    let pass = Breaker::gate(&clean.clone().validate().unwrap(), &CompliantExecutor).unwrap();
    let validated = clean.validate().unwrap();
    breaker::publish(validated, &pass, &gov_for("clean-charter", "support-leads"))
        .expect("a clean authored body must still publish");
}
