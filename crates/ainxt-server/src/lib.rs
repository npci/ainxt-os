...
use ainxt_runtime::{CancelToken, TurnError, TurnSummary};
...
use ainxt_eventlog::{EventLog, LogRecord, SinkStatus};
...
/// Liveness probe (`GET /healthz`): the process is up and its router has finished assembling —
/// nothing more. Deliberately checks no dependency: a liveness poll exists to answer "should this
/// process be restarted?", and a dependency outage is a *readiness* fact ([`readyz_handler`]), never
/// a reason to kill and respawn an otherwise-healthy process (that would just thrash the daemon on
/// top of an already-degraded backend). Unauthenticated, like `/readyz` — a load balancer's liveness
/// prober cannot present a token.
async fn healthz_handler() -> Json<serde_json::Value> {
    Json(serde_json::json!({ "status": "ok" }))
}

/// Readiness probe (`GET /readyz`): a DIFFERENT signal from `/healthz` by design (a CI/load-balancer
/// check that finds the two aliased has caught a real regression here before — see the
/// `ci.yml` comment on the daemon smoke test). Derived from [`EventLog::sink_status`] — the outcome
/// of the daemon's own last durable append — never a synthetic probe write, so polling this endpoint
/// can never itself pollute the tamper-evident chain. `Failing` is the only state that answers
/// unready: `Unknown` (an in-memory/test double, or the legacy `app`/`serve` build with no R3 event
/// log configured at all) is never treated as a fault, matching `sink_status`'s own doc.
///
/// Unauthenticated: a load balancer cannot present a token, and the underlying `io::Error` routinely
/// names a filesystem path, which is not something to hand an anonymous caller — the reason a sink is
/// failing belongs in the daemon's own log, where an authenticated operator reads it.
async fn readyz_handler(State(state): State<AppState>) -> Response {
    let failing = matches!(
        state.event_log.as_ref().map(|log| log.sink_status()),
        Some(SinkStatus::Failing)
    );
    if failing {
        (
...
pub fn app_full_ext(cfg: FullApp, ext: FullAppExt) -> Router {
    ...
    let mut router = Router::new()
        // GAP-FIX build-ci-healthz — liveness/readiness probes. Both the daemon's own startup banner
        // (`ainxt-runtimed: listening on ... /healthz /readyz ...`) and the Console's
        // `wait_until_listening` poller have referenced these paths since R3, but neither was ever
        // actually mounted: a served daemon answered every `/healthz`/`/readyz` request with a bare
        // 404. The Console tolerated it (a documented backward-compat fallback treats ANY successful
        // HTTP response as "ready"), so this stayed invisible there — but it is a hard, correct
        // failure for a strict CI/load-balancer probe that checks the response BODY, not just that
        // *something* answered. See `healthz_handler`/`readyz_handler` below for why the two are kept
        // deliberately distinct rather than one aliasing the other.
...
async fn chat_handler(
    ...
    if let (Some(hub), Some(mut wire_rx)) = (state.wire_hub.clone(), wire_rx) {
        ...
        tokio::spawn(async move {
            ...
            // The shared wire pump is intentionally decoupled from the turn task. In practice the
            // engine can finish the turn just before the pump schedules the terminal envelope. Keep
            // a short post-completion grace window so that a healthy wire event is still preferred,
            // but NEVER leave the HTTP stream hanging forever if the wire seam failed to deliver.
            let mut wire_grace =
                std::pin::pin!(tokio::time::sleep(std::time::Duration::from_millis(250)));
            let mut completion: Option<Result<Result<TurnSummary, TurnError>, ()>> = None;
            loop {
                tokio::select! {
                    biased;
                    ...
                    res = &mut join, if !joined => {
                        joined = true;
                        // Keep the completion result so a missing wire terminal can be projected from
                        // the authoritative TurnSummary instead of silently closing HTTP 200 with zero
                        // bytes.
                        ...
                        // Engine turn: give the shared wire pump a bounded grace period to deliver the
                        // terminal envelope. The response must still make progress if that seam dies.
                    }
                    _ = &mut wire_grace, if joined && !wire_seen => {
                        break;
                    }
                }
            }
            ...
            if bypass || !wire_seen {
                let had_legacy = !legacy_buf.is_empty();
                ...
                // Last-resort transport repair: if the authoritative turn completed successfully but
                // neither the typed wire seam nor the legacy projection produced an event, project the
                // TurnSummary's final answer. This converts the old failure mode — HTTP 200 + an empty
                // SSE stream — into a real answer while preserving the richer wire path whenever it is
                // healthy.
                if !wire_seen && !had_legacy {
                    match completion.as_ref() {
                        Some(Ok(Ok(summary))) if !summary.final_text.is_empty() => {
                            answer.push_str(&summary.final_text);
                            let text_env = EventEnvelope::turn(
                                &session_id,
                                &turn_id,
                                0,
                                &now_rfc3339(),
                                &sha,
                                WireEvent::TextDelta {
                                    text: summary.final_text.clone(),
                                },
                            );
                            let _ = forward_wire_env(
                                &event_log,
                                &session_id,
                                &actor,
                                &out_tx,
                            ...
                            .await;
                            let done_env = EventEnvelope::turn(
                                &session_id,
                                &turn_id,
                                0,
                                &now_rfc3339(),
                        ...
                        Some(Ok(Err(err))) => {
                            let msg = format!("turn failed: {err:?}");
                            let err_env = EventEnvelope::turn(
                                &session_id,
                                &turn_id,
                                0,
                                &now_rfc3339(),
                                &sha,
                                WireEvent::Error(classify_legacy_error(&msg)),
                            );
                            let _ =
                                forward_wire_env(&event_log, &session_id, &actor, &out_tx, err_env)
                                    .await;
                        }
                        Some(Err(())) => {
                            let err_env = EventEnvelope::turn(
                                ...
                                &now_rfc3339(),
                                &sha,
                                WireEvent::Error(ProtocolError::new(
                                    ErrorCategory::ProviderUnavailable,
                                    "turn ended before the runtime could produce an event",
                                )),
                            );
                            let _ =
                                forward_wire_env(&event_log, &session_id, &actor, &out_tx, err_env)
...
/// * `"refused this turn"` — a surface-binding refusal (`ainxt_surface::BindingError` via
///   `AssembledFull::run_turn`'s `"surface '{id}' refused this turn: {e}"`, GAP-FIX
///   build-ci-healthz): role/capability/data-class/department/seniority admission is a policy
///   decision made BEFORE the turn ever reaches a provider, so this must never fall through to the
///   `provider_unavailable` default below — a CI/client that (correctly) treats the two categories
///   differently (one retryable, one not) would otherwise auto-retry a refusal that will never
///   succeed (§6.5.1 `capability_denied`).
...
fn classify_legacy_error(msg: &str) -> ProtocolError {
    ...
    } else if msg.contains("blocked by")
        || msg.contains("budget")
        || msg.contains("refused this turn")
    {
...
mod tests {
    ...
    async fn wire_mem_10_durable_backing_reflects_the_engines_own_writes() {
        ...
        assert!(
            reopened.get_unchecked("p1").is_none(),
    ...
    async fn wire_memory_oki_governance_promote_and_deprecate() {
        ...
        assert_eq!(
            still_draft.get_unchecked("oki-1").unwrap().governance,
            GovernanceState::Draft,
        ...
        assert_eq!(
            after_promote.get_unchecked("oki-1").unwrap().governance,
        ...
        assert_eq!(
            after_deprecate.get_unchecked("oki-1").unwrap().governance,
            GovernanceState::Deprecated
        );
        assert!(!after_deprecate
            .get_unchecked("oki-1")
            .unwrap()
            .is_authoritative());
    }
