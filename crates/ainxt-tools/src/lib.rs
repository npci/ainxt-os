...
/// One unit of scan work handed to the [`scan_worker_pool`]: the chunks to scan, the scanner to run
/// them through (type-erased so one pool serves every [`BoundedArgScanner<S>`] instantiation), and
/// where to send the fused verdict.
struct ScanJob {
    chunks: Vec<String>,
    inner: Arc<dyn ArgClassScanner>,
    result_tx: std::sync::mpsc::Sender<Option<DataClass>>,
}

/// A small, persistent pool of worker threads for [`BoundedArgScanner::classify_args`]. Spawning a
/// brand-new OS thread on every call put the thread-creation itself — not the (trivial) scan work —
/// on the clock against the availability budget: under enough concurrent thread/process creation
/// elsewhere in the same host (many turns in flight, a busy CI runner), `std::thread::spawn`'s own
/// latency could exceed the budget, spuriously tripping the fail-closed path below and escalating a
/// turn that was never actually slow to scan. Reusing already-running workers removes that source of
/// scheduling jitter entirely while leaving the budget/fail-closed contract byte-identical.
fn scan_worker_pool() -> &'static std::sync::mpsc::Sender<ScanJob> {
    static POOL: std::sync::OnceLock<std::sync::mpsc::Sender<ScanJob>> = std::sync::OnceLock::new();
    POOL.get_or_init(|| {
        ...
        for _ in 0..4 {
            ...
            std::thread::spawn(move || loop {
                // Scoped explicitly: the lock must be released the instant `recv()` returns, BEFORE
                // this worker starts processing the job. Folding the lock and the job-processing work
                // into one `while let` scrutinee would hold the guard for the whole loop body (a
                // classic Rust footgun) — a hanging inner scanner on one job would then hold the
                // mutex for its entire duration, starving every other worker of the lock and
                // defeating the pool's entire purpose (nothing else could ever be dequeued).
                let job = {
                ...
                for chunk in &job.chunks {
                    if let Some(c) = job.inner.classify_args(chunk) {
                        cls = Some(cls.map_or(c, |e| e.max(c)));
                    }
                }
                let _ = job.result_tx.send(cls);
            });
        }
        tx
    })
...
impl<S: ArgClassScanner + 'static> ArgClassScanner for BoundedArgScanner<S> {
    fn classify_args(&self, args: &str) -> Option<DataClass> {
        ...
        let inner: Arc<dyn ArgClassScanner> = self.inner.clone();
        let (result_tx, result_rx) = std::sync::mpsc::channel();
        // Hand the bounded chunk scan to the persistent worker pool under the wall-clock budget. A
        // well-behaved scanner returns far within budget; a pathological one cannot pin the calling
        // turn.
        let _ = scan_worker_pool().send(ScanJob {
            chunks,
            inner,
            result_tx,
        });
