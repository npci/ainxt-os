...
licensed under **Apache-2.0**. It is a Rust workspace (63 crates) that ships a network runtime, so
...
- **`crates/ainxt-cli` is not a workspace member.** It is absent from `members` in the root
  `Cargo.toml`, so `--workspace` does not build or test it. Run it explicitly with
  `cargo test --manifest-path crates/ainxt-cli/Cargo.toml` if you touch it.
...
By contributing, you agree your contributions are licensed under Apache-2.0 and that you have
